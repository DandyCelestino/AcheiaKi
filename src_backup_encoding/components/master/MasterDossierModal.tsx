...
export const MasterDossierModal: React.FC<MasterDossierModalProps> = ({
  ...
  const displayRole = targetMerchant ? (targetMerchant.category === 'ServiÃ§os & Profissionais' ? 'PRESTADOR DE SERVIÃ‡OS' : 'LOJISTA / COMÃ‰RCIO') : (targetUser?.role || 'CLIENTE');
      ...
      <div className="bg-white w-full max-w-5xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="bg-slate-100 border-b border-slate-200 px-4 flex items-center space-x-1 overflow-x-auto shrink-0">
          ...
          >
            ...
            <span>VisÃ£o Geral & Cadastro</span>
          </button>
          ...
          {targetMerchant && (
            <>
              ...
              >
                ...
                <span>Vendas & ComissÃµes ({merchantOrders.length})</span>
              </button>
              ...
              >
                ...
                <span>CatÃ¡logo ({merchantProducts.length + merchantServices.length})</span>
              </button>
            </>
          )}
          ...
          >
            ...
            <span>Logs & ManifestaÃ§Ãµes ({targetAuditLogs.length})</span>
          </button>
        </div>
        ...
        <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {/* 1. VISÃƒO GERAL & CADASTRO COMPLETO */}
          {activeTab === 'overview' && (
                ...
                {targetMerchant && (
                  <>
                    ...
                    <div className="p-3.5 bg-purple-50 border border-purple-200 rounded-xl">
                      <span className="text-[10px] font-bold text-purple-800 uppercase tracking-wider block">ComissÃ£o Retida Master</span>
                    ...
                    <div className="p-3.5 bg-blue-50 border border-blue-200 rounded-xl">
                      <span className="text-[10px] font-bold text-blue-800 uppercase tracking-wider block">Repasse LÃ­quido Ã  Loja</span>
                      ...
                      <span className="text-[10px] text-blue-700">ApÃ³s deduÃ§Ã£o da comissÃ£o</span>
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-3">
                  <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                    ...
                    <span>Dados Pessoais & DocumentaÃ§Ã£o</span>
                  </h4>

                  <div className="space-y-2 text-xs divide-y divide-slate-100">
                    <div className="flex justify-between pt-1">
                      <span className="text-slate-500">Nome Completo:</span>
                      <span className="font-bold text-slate-900">{targetUser?.name || targetMerchant?.ownerName || 'NÃ£o informado'}</span>
                    </div>
                    <div className="flex justify-between pt-1.5">
                      <span className="text-slate-500">CPF / CNPJ:</span>
                      <span className="font-sans font-bold text-slate-900">{targetUser?.cpf || targetMerchant?.cnpjOrCpf || 'NÃ£o cadastrado'}</span>
                    </div>
                    <div className="flex justify-between pt-1.5">
                    ...
                    {targetUser?.secondaryPhone && (
                      <div className="flex justify-between pt-1.5">
                        <span className="text-slate-500">Telefone SecundÃ¡rio:</span>
                    ...
                    <div className="flex justify-between pt-1.5">
                      ...
                      <span className="text-slate-800">{targetUser?.birthDate || 'NÃ£o informada'}</span>
                    </div>
                    <div className="flex justify-between pt-1.5">
                      <span className="text-slate-500">GÃªnero:</span>
                      <span className="text-slate-800">{targetUser?.gender || 'NÃ£o especificado'}</span>
                    </div>
                    <div className="flex justify-between pt-1.5">
                      <span className="text-slate-500">EndereÃ§o Completo:</span>
                ...
                {/* Dados do Estabelecimento (se aplicÃ¡vel) */}
                {targetMerchant ? (
                  <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-3">
                    <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                      <Store className="w-4 h-4 text-blue-600" />
                      <span>InformaÃ§Ãµes do Estabelecimento</span>
                    </h4>

                    <div className="space-y-2 text-xs divide-y divide-slate-100">
                      <div className="flex justify-between pt-1">
                        <span className="text-slate-500">RazÃ£o Social / Nome Fantasia:</span>
                        <span className="font-bold text-slate-900">{targetMerchant.name}</span>
                      </div>
                      <div className="flex justify-between pt-1.5">
                        <span className="text-slate-500">Categoria de AtuaÃ§Ã£o:</span>
                        <span className="font-bold text-blue-700">{targetMerchant.category}</span>
                      </div>
                      <div className="flex justify-between pt-1.5">
                        <span className="text-slate-500">HorÃ¡rio de Funcionamento:</span>
                        <span className="text-slate-800">{targetMerchant.openingHours || '08:00 Ã s 18:00'}</span>
                      </div>
                      <div className="flex justify-between pt-1.5">
                        <span className="text-slate-500">Taxa de Entrega PadrÃ£o:</span>
                        <span className="font-bold text-slate-900">
                          {targetMerchant.deliveryFee ? `R$ ${targetMerchant.deliveryFee.toFixed(2)}` : 'GrÃ¡tis / Sob Consulta'}
                        </span>
                      </div>
                      <div className="flex justify-between pt-1.5">
                      ...
                      <div className="flex justify-between pt-1.5">
                        <span className="text-slate-500">ComissÃ£o da Plataforma:</span>
                        <span className="font-sans font-black text-purple-700">{commissionRate}%</span>
                      </div>
                    </div>
                  </div>
                ) : (
                  /* PreferÃªncias de VIP / Medidas do Cliente */
                  <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-3">
                    <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider flex items-center gap-1.5">
                      <ShoppingBag className="w-4 h-4 text-emerald-600" />
                      <span>Perfil VIP & PreferÃªncias de Consumo</span>
                    </h4>

                    <div className="space-y-2 text-xs divide-y divide-slate-100">
                      <div className="flex justify-between pt-1">
                        <span className="text-slate-500">Tamanho de Roupas (Superior):</span>
                        <span className="font-bold text-slate-900">{targetUser?.measurements?.topSize || 'M (PadrÃ£o)'}</span>
                      </div>
                      <div className="flex justify-between pt-1.5">
                        <span className="text-slate-500">Tamanho de CalÃ§ados:</span>
            ...
            <div className="space-y-5">
              <div className="p-4 bg-purple-50 border border-purple-200 rounded-xl flex items-start space-x-3">
                ...
                <div className="text-xs text-purple-950 space-y-1">
                  ...
                  <p>
                    As contas sÃ£o individuais e resguardadas sob absoluto sigilo entre usuÃ¡rios e comerciantes.
                    Apenas vocÃª, como Master, possui acesso a credenciais, dados de identificaÃ§Ã£o e redefiniÃ§Ãµes de acesso.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-4">
                <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider">
                  Credenciais de AutenticaÃ§Ã£o do UsuÃ¡rio
                </h4>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center space-x-2">
                      <input
                        ...
                        readOnly
                        value={targetUser?.password || 'â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢'}
                        className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-sans font-bold text-slate-900"
                      />
                      <button
                        type="button"
                ...
                {targetMerchant && (
                  <div className="pt-4 border-t border-slate-200">
                    <h5 className="text-xs font-bold text-slate-800 mb-2">ComissÃ£o Personalizada da Loja / Prestador</h5>
          ...
          {/* 3. HISTÃ“RICO DE COMPRAS (COMO CLIENTE) */}
          {activeTab === 'orders' && (
            <div className="space-y-3">
              <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider">
                HistÃ³rico de Pedidos Realizados ({customerOrders.length})
              </h4>

              {customerOrders.length === 0 ? (
                <div className="p-8 text-center bg-slate-50 border border-slate-200 rounded-xl text-slate-500 text-xs">
                  Nenhum pedido realizado por este usuÃ¡rio atÃ© o momento.
                </div>
              ) : (
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <p className="text-slate-600">
                          Loja: <strong>{order.merchantName}</strong> â€¢ {order.itemsSummary || '1 item'}
                        </p>
                        <p className="text-[11px] text-slate-400">
                          {order.createdAt || 'Data recente'} â€¢ Entrega: {order.deliveryAddress || 'Retirada'}
                        </p>
                      </div>
          ...
          {/* 4. VENDAS & COMISSÃ•ES (COMO LOJA / PRESTADOR) */}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-slate-500">
                  ComissÃ£o Base: <strong className="text-purple-700">{commissionRate}%</strong>
                </span>
              </div>
                    ...
                    return (
                      ...
                      >
                        <div className="space-y-1">
                          ...
                          <p className="text-slate-600">
                            Itens: {order.itemsSummary || 'Itens diversos'} â€¢ Status: <strong>{order.status}</strong>
                          </p>
                        ...
                        <div className="grid grid-cols-3 gap-2 text-right bg-slate-50 p-2 rounded-lg border border-slate-100">
                          ...
                          <div>
                            <span className="text-[9px] text-purple-700 block uppercase font-bold">ComissÃ£o Master</span>
                            <span className="font-black text-purple-800">
                              {commission.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                            </span>
                          </div>
                          <div>
                            <span className="text-[9px] text-emerald-700 block uppercase font-bold">LÃ­quido Loja</span>
          ...
          {/* 5. CATÃLOGO DE PRODUTOS & SERVIÃ‡OS */}
          {activeTab === 'catalog' && targetMerchant && (
            <div className="space-y-4">
              <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider">
                CatÃ¡logo de Produtos & ServiÃ§os ({merchantProducts.length + merchantServices.length})
              </h4>
                  ...
                  <div key={srv.id} className="p-3 bg-white border border-slate-200 rounded-xl flex space-x-3 items-center">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <span className="text-xs font-black text-purple-700">
                        {srv.price ? srv.price.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : 'Sob OrÃ§amento'}
                      </span>
          ...
          {activeTab === 'audit' && (
            <div className="space-y-3">
              <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider">
                Eventos e ManifestaÃ§Ãµes Registradas no Sistema ({targetAuditLogs.length})
              </h4>

              {targetAuditLogs.length === 0 ? (
                <div className="p-8 text-center bg-slate-50 border border-slate-200 rounded-xl text-slate-500 text-xs">
                  Nenhum evento crÃ­tico registrado especificamente para este cadastro.
                </div>
              ) : (
                <div className="space-y-2">
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
          <div className="text-[11px] text-slate-500">
            DossiÃª de Auditoria Master â€¢ Sigilo Total Garantido
          </div>
          <button
            type="button"
            onClick={onClose}
