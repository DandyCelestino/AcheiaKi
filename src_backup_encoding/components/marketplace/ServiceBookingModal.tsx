...
export const ServiceBookingModal: React.FC<ServiceBookingModalProps> = ({
  ...
  const [selectedProfessional, setSelectedProfessional] = useState(
    service?.professionals[0] || 'Profissional DisponÃ­vel'
  );
  ...
  const handleConfirmBooking = (e: React.FormEvent) => {
    ...
    const newBooking = createOrder({
      ...
      customerAddress:
        selectedLocation === 'DOMICILIO'
          ? currentUser?.address || 'EndereÃ§o em Cachoeiras de Macacu (A Combinar)'
      ...
      serviceDetails: {
        ...
        merchantResponse: {
          status: 'PENDENTE',
          responseMessage: 'SolicitaÃ§Ã£o enviada. Aguardando confirmaÃ§Ã£o e orientaÃ§Ãµes do lojista/profissional.',
    ...
    triggerToast(`SolicitaÃ§Ã£o de agendamento ${newBooking.code} enviada para o prestador!`);
  };
        ...
        <div className="bg-slate-900 text-white p-4 sm:p-5 flex items-center justify-between">
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-slate-400">â€¢ {service.category}</span>
            </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 text-slate-800 space-y-5">
          {confirmedBooking ? (
            <div className="text-center space-y-4 py-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 mt-1 max-w-md mx-auto">
                  Sua solicitaÃ§Ã£o foi encaminhada para <strong>{service.merchantName}</strong>. 
                  VocÃª receberÃ¡ a confirmaÃ§Ã£o e as orientaÃ§Ãµes oficiais diretamente na aba 
              ...
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl text-left space-y-2.5 text-xs">
                <div className="flex justify-between font-bold text-slate-900 border-b border-slate-200 pb-2">
                  <span>CÃ³digo da SolicitaÃ§Ã£o:</span>
                ...
                <div className="flex justify-between text-slate-700">
                  ...
                  <span className="font-semibold text-slate-900">
                    {selectedLocation === 'ESTABELECIMENTO'
                      ? 'No ConsultÃ³rio / Estabelecimento'
                      : selectedLocation === 'DOMICILIO'
                      ? 'Em DomicÃ­lio (MÃ³vel)'
                      : 'Online / Remoto'}
                  </span>
                </div>
                <div className="flex justify-between text-slate-700">
                  <span>Data & HorÃ¡rio:</span>
                ...
                <div className="flex justify-between text-slate-700 pt-1 border-t border-slate-200">
                  ...
                  <span className="font-black text-slate-900 text-sm">
                    ...
                    <span className="text-[10px] font-normal text-slate-500 ml-1">
                      ({selectedPricingType === 'HORA' ? 'por Hora' : selectedPricingType === 'DIARIA' ? 'por DiÃ¡ria' : selectedPricingType === 'MENSAL' ? 'Mensalidade' : 'ServiÃ§o Fixo'})
                    </span>
                  </span>
                </div>
              </div>
              ...
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-900 text-left flex items-start space-x-2">
                ...
                <div>
                  <strong className="block text-blue-950 font-bold">ConfirmaÃ§Ã£o & Resposta do Prestador:</strong>
                  <span>O prestador irÃ¡ validar a vaga na agenda e poderÃ¡ incluir recomendaÃ§Ãµes de preparo ou documentos para o seu atendimento.</span>
                </div>
              </div>
            ...
            <form onSubmit={handleConfirmBooking} className="space-y-5">
              {/* Tabela de PreÃ§os & Modalidades de ContrataÃ§Ã£o (Hora / Dia / MÃªs) */}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1.5 flex items-center space-x-1.5">
                  ...
                  <span>1. Onde vocÃª prefere o atendimento?</span>
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  ...
                  >
                    ...
                    <div className="font-bold">Em DomicÃ­lio</div>
                    <div className="text-[10px] text-slate-500 font-normal">
                      Prestador vai atÃ© vocÃª em Macacu
                    </div>
                  </button>
                  ...
                  >
                    ...
                    <div className="font-bold">Online / VÃ­deo</div>
                    <div className="text-[10px] text-slate-500 font-normal">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {service.professionals.map((prof) => (
                      ...
                      >
                        ...
                        {selectedProfessional === prof && (
                          <span className="text-[10px] font-bold text-blue-600">âœ“</span>
                        )}
                      </button>
                    ))}
                  </div>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-slate-900 flex items-center space-x-1.5">
                    ...
                    <span>3. Escolha a Data & HorÃ¡rio Vago da Agenda:</span>
                  </label>
              ...
              {/* 4. Dados do Cliente & ObservaÃ§Ãµes */}
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">
                    ObservaÃ§Ãµes ou DescriÃ§Ã£o da Necessidade (Opcional):
                  </label>
                  <textarea
                    rows={2}
                    value={customerNotes}
                    onChange={(e) => setCustomerNotes(e.target.value)}
                    placeholder="Ex: Descrever sintomas, necessidades especÃ­ficas, matÃ©rias de reforÃ§o ou tipo de reparo..."
              ...
              <div className="p-4 bg-slate-900 text-white rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <span className="text-[11px] text-slate-400 block uppercase font-bold tracking-wider">
                    Valor da ContrataÃ§Ã£o ({selectedPricingType}):
                  </span>
