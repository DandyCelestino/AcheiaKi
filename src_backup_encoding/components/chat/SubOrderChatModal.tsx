...
export const SubOrderChatModal: React.FC = () => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
        <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-rose-100 text-center">
          ...
          <p className="text-xs text-slate-500 mb-5 leading-relaxed">
            VocÃª nÃ£o possui permissÃ£o para visualizar mensagens ou dados vinculados a este subpedido. Somente o comprador titular, a loja responsÃ¡vel e a auditoria Master possuem autorizaÃ§Ã£o para acessar esta conversa.
          </p>
          <button
  ...
  const currentSenderName =
    currentUser?.name ||
    (isUserSeller ? merchantName : isUserMaster ? 'AdministraÃ§Ã£o Achei Aqui' : 'Cliente');
  ...
  const handleSendMessage = (e?: React.FormEvent) => {
    ...
    // Enviar notificaÃ§Ã£o in-app particular para a contraparte direta
    ...
    if (isUserClient && activeChatSubOrder.merchantId) {
      sendInAppNotification({
        title: `ðŸ’¬ Nova Mensagem de ${currentSenderName}`,
        message: `OlÃ¡, ${merchantName}!\n\nVocÃª recebeu uma nova mensagem de ${currentSenderName} referente a "${orderTitle || 'atendimento'}":\n"${msgSnippet}"`,
        category: 'COMUNICADO',
        priority: 'HIGH',
      ...
      sendInAppNotification({
        title: `ðŸ’¬ Resposta de ${merchantName}`,
    ...
    triggerToast('ðŸ’¬ Mensagem interna enviada com sucesso!');
  };
  ...
  const quickChips = isUserSeller
    ? [
        'âœ… Pedido separado e pronto para retirada!',
        'ðŸ›µ Seu pedido saiu para entrega com o motoboy!',
        'ðŸ’³ Aguardamos a confirmaÃ§Ã£o do pagamento via PIX.',
        'ðŸ“¦ Estamos preparando o seu pacote com carinho.',
        'â° HorÃ¡rio confirmado para o seu atendimento!'
      ]
    : [
        'ðŸ›µ OlÃ¡! Gostaria de saber a previsÃ£o de entrega/retirada.',
        'ðŸ“ JÃ¡ estou a caminho do balcÃ£o para retirar.',
        'ðŸ’³ Pagamento realizado via PIX!',
        'â“ OlÃ¡, gostaria de confirmar mais detalhes deste pedido.'
      ];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/75 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col h-[90vh] max-h-[750px]">
          ...
          <div className="flex items-center space-x-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-slate-300 font-semibold truncate">
                {merchantName} <span className="text-slate-500 font-normal">â†”</span> {customerName}
              </p>
              {orderTitle && (
          ...
          <div className="flex items-center space-x-2 shrink-0">
            {securityCode && (
              <div className="hidden sm:flex flex-col items-end px-2.5 py-1 bg-slate-800 rounded-lg border border-slate-700">
                <span className="text-[9px] text-slate-400 font-bold uppercase">CÃ³d. SeguranÃ§a</span>
        ...
        <div className="px-4 py-2 bg-emerald-50 border-b border-emerald-200 text-emerald-900 text-[11px] flex items-center justify-between gap-2 shrink-0">
          <div className="flex items-center space-x-1.5 min-w-0">
            ...
            <span className="truncate">
              <strong>Canal Oficial de Mensagem Interna:</strong> Conversas e comprovantes sÃ£o vinculados a este subpedido com seguranÃ§a jurÃ­dica.
            </span>
          </div>
        ...
        {/* PRODUCT CONTEXT CARD (SE ABERTO A PARTIR DE UM PRODUTO OU SERVIÃ‡O) */}
            ...
            <div className="text-center py-10 space-y-3">
              ...
              <div>
                <p className="font-bold text-slate-800 text-sm">
                  InÃ­cio da Conversa do Subpedido {codigoSubpedido}
                </p>
                <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                  Envie uma mensagem abaixo para tirar dÃºvidas sobre entrega, retirada, pagamento ou orientaÃ§Ãµes do produto.
                </p>
              </div>
            </div>
          ) : (
            ...
            <div className="flex items-center justify-between text-xs text-slate-600 px-1">
              <label className="flex items-center space-x-1.5 cursor-pointer">
                ...
                <span className="font-semibold text-[11px] flex items-center space-x-1 text-amber-900">
                  ...
                  <span>Nota Interna (VisÃ­vel apenas para Lojista e Master)</span>
                </span>
              </label>
            </div>
          )}
