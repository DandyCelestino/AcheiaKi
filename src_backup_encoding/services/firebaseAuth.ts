...
export async function firebaseLoginWithEmail(
  ...
  try {
    ...
    if (user.status === 'blocked' || user.status === 'suspended') {
      ...
      return {
        success: false,
        message: `Acesso suspenso ou bloqueado: ${user.statusReason || 'Entre em contato com a administraÃ§Ã£o Achei Aqui.'}`,
      };
    }

    // ValidaÃ§Ã£o 2FA para perfis com permissÃ£o elevada
    ...
    if (isHighPrivilege) {
      ...
      return {
        ...
        message: `CÃ³digo de verificaÃ§Ã£o em 2 etapas gerado para ${user.phone || user.email}.`,
      };
    }
