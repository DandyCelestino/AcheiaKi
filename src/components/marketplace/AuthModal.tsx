...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleRegisterCustomer = async (e: React.FormEvent) => {
    ...
    const customerResult = await registerCustomerWithFirebase({
    ...
    if (!customerResult.success) {
      setErrorMessage(customerResult.message || 'Não foi possível concluir o cadastro.');
      return;
    }
    onClose();
  };
  ...
  const handleRegisterMerchant = async (e: React.FormEvent) => {
    ...
    try {
      const newMerchant = await registerMerchant(
        {
      ...
      setSuccessMessage('Cadastro realizado com sucesso! Sua conta foi salva. Agora conclua o pagamento para liberar o painel.');
      triggerToast('Cadastro realizado com sucesso! Checkout de pagamento liberado.');
      setTab('plan-checkout');
    } catch (error: any) {
      console.error('[CADASTRO LOJISTA]', error);
      setErrorMessage(error?.message || 'Não foi possível concluir o cadastro. Verifique os dados e tente novamente.');
    }
  };
