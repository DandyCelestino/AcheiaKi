...
export const DeliveryDriverDashboard: React.FC = () => {
  ...
  // Submeter confirmação com código de 4 dígitos do cliente
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {activeDriver?.name || 'Entregador Parceiro'} • {activeDriver?.vehicleType} ({activeDriver?.vehiclePlate})
              </p>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto w-full px-4 sm:px-6 pt-4">
        {activeDriver?.status === 'PENDENTE' && (
          <div className="bg-amber-950/50 border border-amber-500/40 rounded-2xl p-4 text-xs text-amber-200 flex items-start space-x-3 mb-4">
            ...
            <div>
              <p className="font-bold text-amber-300 text-sm">Cadastro em Análise pela Administração Master</p>
              <p className="text-amber-200/80 mt-1 leading-relaxed">
                Seus documentos (CNH e veículo) estão sendo verificados. Assim que aprovado pelo Master, você poderá ficar Online e receber corridas remuneradas em Cachoeiras de Macacu.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="bg-red-950/60 border border-red-500/40 rounded-2xl p-4 text-xs text-red-200 flex items-start space-x-3 mb-4">
            ...
            <div>
              ...
              <p className="text-red-200/80 mt-1 leading-relaxed">
                Motivo: {activeDriver.statusReason || 'Entre em contato com o suporte Master para esclarecer pendências operacionais.'}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
          ...
          <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4">
            ...
            <p className="text-[10px] text-slate-500 mt-1">Concluídas com sucesso</p>
          </div>
          ...
          <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-4">
            ...
            <p className="text-xl sm:text-2xl font-black text-yellow-400">
              {(activeDriver?.rating || 5.0).toFixed(1)} ★
            </p>
        ...
        <div className="flex border-b border-slate-800 space-x-2 mb-6">
          ...
          >
            ...
            <span>Meu Histórico de Ganhos ({myCompletedRides.length})</span>
          </button>
          ...
          >
            ...
            <span>Meu Cadastro & Veículo</span>
          </button>
        </div>
        ...
        {activeSubTab === 'radar' && (
          <div className="space-y-6">
            {/* CORRIDA ATIVA EM ANDAMENTO (MÁXIMA PRIORIDADE) */}
            {activeRide ? (
              <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950/40 border-2 border-emerald-500/50 rounded-3xl p-5 sm:p-6 shadow-xl shadow-emerald-950/30 space-y-5">
                ...
                <div className="p-4 bg-slate-950/70 border border-slate-800 rounded-2xl flex flex-wrap items-center justify-between gap-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase font-bold block">Distância Calculada:</span>
                  ...
                  <div className="flex items-center space-x-3">
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase font-bold block">Remuneração do Entregador:</span>
                ...
                {/* BOTÕES DE PROGRESSÃO OPERACIONAL DO CICLO DE ENTREGA */}
                <div className="pt-2 space-y-3">
                  {activeRide.status === 'ACEITA' && (
                    ...
                    >
                      <Navigation className="w-4 h-4" />
                      <span>INICIAR DESLOCAMENTO ATÉ O ESTABELECIMENTO</span>
                    </button>
                  )}
                    ...
                    <div className="space-y-3">
                      {!isConfirmingDelivery ? (
                        ...
                        >
                          ...
                          <span>CHEGUEI NO DESTINO: DIGITAR CÓDIGO DO CLIENTE</span>
                        </button>
                      ) : (
                        <form
                        ...
                        >
                          <div className="text-center space-y-1">
                            <span className="text-xs text-emerald-400 font-black uppercase tracking-wider">
                              Validação de Entrega Segura
                            </span>
                            <h4 className="text-base font-bold text-white">
                              Solicite o código de 4 dígitos ao cliente
                            </h4>
                            <p className="text-xs text-slate-400">
                              O cliente possui este código na tela de acompanhamento de pedidos dele.
                            </p>
                          </div>
            ...
            {/* RADAR DE CORRIDAS DISPONÍVEIS */}
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-black text-white flex items-center space-x-2">
                    <span>Corridas Disponíveis no Radar</span>
                    <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 text-xs font-bold">
                      {availableRides.length}
                    </span>
                  </h3>
                  <p className="text-xs text-slate-400">
                    Cachoeiras de Macacu, RJ • Toque para aceitar e garantir a corrida
                  </p>
                </div>
              </div>
              ...
              {!isOnline ? (
                <div className="p-8 text-center bg-slate-950/60 border border-slate-800 rounded-3xl space-y-3">
                  ...
                  <h4 className="font-bold text-white text-base">Você está Offline</h4>
                  <p className="text-xs text-slate-400 max-w-md mx-auto">
                    Ative o botão "ONLINE" no topo da tela para visualizar e aceitar corridas em tempo real.
                  </p>
                  {isApproved && (
                    <button
                      type="button"
                ...
                <div className="p-8 text-center bg-slate-950/60 border border-slate-800 rounded-3xl space-y-3">
                  ...
                  <p className="text-xs text-slate-400 max-w-md mx-auto">
                    Nenhuma nova entrega pendente no momento. Assim que um lojista finalizar um pedido e solicitar delivery, ela aparecerá automaticamente aqui.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between gap-3">
                        ...
                        >
                          ...
                          <span>{activeRide ? 'Já em corrida' : 'ACEITAR CORRIDA'}</span>
                        </button>
                      </div>
                    </div>
                  ))}
        ...
        {/* 6. TAB: HISTÓRICO DE ENTREGAS */}
        {activeSubTab === 'history' && (
          <div className="bg-slate-950/80 border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-4">
            <div>
              <h3 className="text-lg font-black text-white">Histórico de Entregas & Ganhos</h3>
              <p className="text-xs text-slate-400">
                Registro oficial de corridas finalizadas com confirmação por código.
              </p>
            </div>

            {myCompletedRides.length === 0 ? (
              <div className="p-8 text-center text-slate-500 text-xs">
                Nenhuma corrida finalizada ainda. Suas entregas concluídas aparecerão aqui com os valores creditados.
              </div>
            ) : (
        ...
        {/* 7. TAB: PERFIL & VEÍCULO */}
        {activeSubTab === 'profile' && activeDriver && (
          <div className="bg-slate-950/80 border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-6">
            <div>
              <h3 className="text-lg font-black text-white">Dados do Entregador & Veículo</h3>
              <p className="text-xs text-slate-400">
                Informações cadastradas para faturamento e autorização de transporte.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-1">
                <span className="text-slate-500 text-[10px] uppercase font-bold block">Veículo Credenciado:</span>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-slate-950 border border-slate-800 rounded-3xl max-w-md w-full p-6 space-y-4">
            <div className="flex items-center space-x-2 text-amber-400">
              ...
              <h3 className="font-black text-white text-base">Registrar Ocorrência na Corrida</h3>
            </div>
            <p className="text-xs text-slate-400">
              Descreva o imprevisto (ex: endereço incorreto, loja fechada, cliente ausente). O alerta será encaminhado imediatamente à equipe Master.
            </p>
            ...
            <textarea
              ...
              placeholder="Descreva o motivo da ocorrência com clareza..."
