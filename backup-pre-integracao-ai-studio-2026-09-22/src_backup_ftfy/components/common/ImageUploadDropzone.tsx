  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      if (validFiles.length === 0) {
        setErrorMessage('Nenhum arquivo de imagem vÃ¡lido selecionado. Por favor, envie arquivos JPG, PNG ou WEBP.');
  ...
  const handleApplyUrl = (e: React.FormEvent) => {
    ...
    if (multiple) {
      if (imagesList.length >= maxImages) {
        setErrorMessage(`Limite mÃ¡ximo de ${maxImages} imagens atingido.`);
        return;
      }
        ...
        >
          {isProcessing ? (
            <div className="py-4 space-y-2 flex flex-col items-center">
              ...
              <p className="text-[10px] text-slate-500">Ajustando resoluÃ§Ã£o para carregamento ultrarrÃ¡pido</p>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-1 max-w-sm">
                ...
                <p className="text-[11px] text-slate-500">
                  {multiple
                    ? `Selecione atÃ© ${maxImages} fotos da sua galeria ou arraste os arquivos aqui.`
                    : 'Arraste e solte o arquivo aqui ou use os botÃµes abaixo.'}
                </p>
              </div>
            ...
            <form onSubmit={handleApplyUrl} className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[11px] font-bold text-slate-700 flex items-center gap-1">
                  ...
                  <span>Colar URL de imagem pÃºblica:</span>
                </label>
                <button
                  type="button"
