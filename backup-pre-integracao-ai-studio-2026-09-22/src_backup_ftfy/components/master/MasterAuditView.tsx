...
export const MasterAuditView: React.FC = () => {
  ...
  const getCategoryBadge = (cat?: AuditCategory) => {
    switch (cat) {
      case 'SECURITY':
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-purple-100 text-purple-800">SeguranÃ§a</span>;
      case 'DATA_PRIVACY':
      ...
      case 'USER_MANAGEMENT':
        return <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-cyan-100 text-cyan-800">UsuÃ¡rios</span>;
      default:
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Registro imutÃ¡vel de mensagens em subpedidos, mudanÃ§as de status, liberaÃ§Ãµes de dados do comprador e intervenÃ§Ãµes master.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={() => {
              if (
                window.confirm(
                  'Deseja arquivar e reiniciar a lista de logs? Esta aÃ§Ã£o gerarÃ¡ um registro de auditoria AUDIT_LOGS_PURGE.'
                )
              ) {
                clearAuditLogs();
              }
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
        ...
        <div className="bg-white p-4 rounded-xl border border-rose-200/80 bg-rose-50/20 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-rose-700">LiberaÃ§Ã£o de Dados (LGPD)</span>
        ...
        <div className="bg-white p-4 rounded-xl border border-indigo-200/80 bg-indigo-50/20 shadow-xs">
          ...
          <span className="text-[10px] text-indigo-500 font-medium">InteraÃ§Ãµes em subpedidos</span>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row items-center gap-3">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder="Pesquisar por aÃ§Ã£o, usuÃ¡rio, ID de pedido, IP, detalhes..."
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          >
            ...
            <option value="COMMUNICATION">ComunicaÃ§Ã£o / Chat</option>
            <option value="ORDER">Pedidos & Estoque</option>
            <option value="FINANCIAL">Financeiro & Taxas</option>
            <option value="USER_MANAGEMENT">UsuÃ¡rios & Lojas</option>
          ...
          >
            ...
            <option value="CRITICAL">CrÃ­tica / Alta</option>
            <option value="SECURITY">SeguranÃ§a</option>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50/80 text-slate-600 font-semibold uppercase text-[10px] tracking-wider border-b border-slate-200/80">
              <tr>
                ...
                <th className="px-4 py-3.5">AÃ§Ã£o / Evento</th>
                <th className="px-4 py-3.5">UsuÃ¡rio / Papel</th>
                  ...
                  <tr key={`${log.id}-${idx}`} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3.5">
                      ...
                      <div className="text-[10px] text-slate-400 flex items-center gap-1">
                        ...
                        {log.userId && <span>â€¢ ID: {log.userId}</span>}
                      </div>
                    </td>
