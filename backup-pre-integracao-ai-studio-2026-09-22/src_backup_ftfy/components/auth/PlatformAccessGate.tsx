...
export const PlatformAccessGate: React.FC<PlatformAccessGateProps> = ({ onSuccess }) => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.requires2FA) {
        ...
        setSuccessMessage(result.message || 'CÃ³digo de confirmaÃ§Ã£o em 2 etapas enviado com sucesso.');
        return;
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setErrorMessage(result.message || 'NÃ£o foi possÃ­vel concluir o login com o Google.');
      }
    } catch (err: any) {
      setErrorMessage(err?.message || 'Falha na autenticaÃ§Ã£o Google.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleVerify2FASubmit = (e: React.FormEvent) => {
    ...
    if (result.success) {
      ...
      setErrorMessage(result.message || 'CÃ³digo de 2Âª etapa invÃ¡lido.');
    }
  };
  ...
  const handleSendResetCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(err?.message || 'Falha ao solicitar recuperaÃ§Ã£o.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleFinishPasswordReset = (e: React.FormEvent) => {
    ...
    if (!resetCode || resetCode.length < 6) {
      setErrorMessage('Digite o cÃ³digo de verificaÃ§Ã£o de 6 dÃ­gitos.');
      return;
    }
    ...
    if (newResetPassword !== confirmResetPassword) {
      setErrorMessage('A confirmaÃ§Ã£o de senha nÃ£o confere com a nova senha.');
      return;
    }

    const res = completePasswordReset(forgotEmail.trim(), resetCode.trim(), newResetPassword);
    if (res.success) {
      setSuccessMessage('Senha atualizada com sucesso! Realizando login automÃ¡tico...');
  ...
  const handleCustomerRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!customerTermsAccepted) {
      setErrorMessage('VocÃª deve aceitar os Termos de Uso e Privacidade.');
      return;
    }
  ...
  const handleMerchantRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (!merchantStoreName.trim() || !merchantOwnerName.trim() || !merchantEmail.trim()) {
      setErrorMessage('Preencha todos os campos obrigatÃ³rios do negÃ³cio.');
      return;
    }
    ...
    if (!merchantTermsAccepted) {
      setErrorMessage('VocÃª deve concordar com os Termos de Credenciamento Achei Aqui.');
      return;
    }
    ...
    try {
      const res = await registerMerchantWithFirebase({
        ...
        description: merchantDesc.trim() || (isService ? 'Prestador com atendimento e referÃªncias verificadas em Cachoeiras de Macacu.' : 'Estabelecimento local oficial em Cachoeiras de Macacu.'),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-900 flex flex-col justify-between text-slate-100 font-sans selection:bg-emerald-600 selection:text-white relative overflow-x-hidden">
      ...
      <header className="px-4 sm:px-8 py-4 border-b border-emerald-900/40 bg-slate-950/60 backdrop-blur-md flex items-center justify-between z-10">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-emerald-400/80 font-medium">ðŸƒ Cachoeiras de Macacu - RJ</p>
          </div>
        </div>
        ...
        <div className="w-full max-w-xl bg-slate-900/90 backdrop-blur-xl border border-emerald-800/40 rounded-3xl p-5 sm:p-8 shadow-2xl shadow-black/80 relative overflow-hidden">
          ...
          <div className="text-center space-y-2 mb-6">
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
              ...
              {activeTab === 'forgot-password' && 'RecuperaÃ§Ã£o de Senha'}
            ...
            <p className="text-xs sm:text-sm text-slate-300 max-w-md mx-auto leading-relaxed">
              {activeTab === 'login' && 'O acesso ao shopping e serviÃ§os de Cachoeiras de Macacu Ã© exclusivo para usuÃ¡rios autenticados com login e senha.'}
              {activeTab === 'forgot-password' && 'Redefina sua senha com seguranÃ§a atravÃ©s do cÃ³digo de verificaÃ§Ã£o enviado ao seu e-mail.'}
              {activeTab === 'register-customer' && 'Crie sua conta gratuita em menos de 1 minuto para comprar, agendar serviÃ§os e acompanhar pedidos.'}
              {activeTab === 'register-merchant' && 'Cadastre seu estabelecimento ou serviÃ§os para vender e agendar clientes em toda a regiÃ£o.'}
            </p>
          </div>
          ...
          {activeTab === 'login' && !is2FAStep && (
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-center space-x-1.5 pt-1 text-[11px] text-emerald-400 font-medium">
                ...
                <span>AutenticaÃ§Ã£o Oficial Firebase (Projeto acheiaki-e25d6)</span>
              </div>
            </form>
          )}
            ...
            <form onSubmit={handleVerify2FASubmit} className="space-y-4">
              <div className="p-4 bg-emerald-950/40 border border-emerald-800/60 rounded-2xl space-y-2">
                <div className="flex items-center space-x-2 text-emerald-300 text-xs font-bold">
                  <ShieldCheck className="w-4 h-4 text-emerald-400" />
                  <span>ConfirmaÃ§Ã£o de SeguranÃ§a em 2 Etapas (2FA)</span>
                </div>
                <p className="text-xs text-slate-300 leading-relaxed">
                  Para sua proteÃ§Ã£o, geramos um cÃ³digo de seguranÃ§a de 6 dÃ­gitos para o e-mail <strong>{pending2FAEmail}</strong>.
                </p>
                {simulated2FACode && (
                  <div className="p-2.5 bg-slate-950 rounded-xl border border-emerald-700/50 flex items-center justify-between mt-2">
                    <span className="text-xs font-bold text-emerald-400">CÃ³digo 2FA:</span>
              ...
              >
                <span>CONFIRMAR CÃ“DIGO E ACESSAR</span>
          ...
          {/* ==================================================== */}
          {/* TAB 2: RECUPERAÃ‡ÃƒO DE SENHA (MECANISMO COMPLETO) */}
          {/* ==================================================== */}
          {activeTab === 'forgot-password' && (
              ...
              {resetStep === 1 ? (
                <form onSubmit={handleSendResetCode} className="space-y-4">
                  <div className="p-3.5 bg-emerald-950/40 border border-emerald-800/40 rounded-2xl text-xs text-slate-300 leading-relaxed">
                    <p>
                      Informe o e-mail da sua conta. Enviaremos um <strong>cÃ³digo de recuperaÃ§Ã£o de 6 dÃ­gitos</strong> para autorizar a criaÃ§Ã£o de uma nova senha.
                    </p>
                  </div>
                  ...
                  <div className="text-center pt-2">
                    ...
                    >
                      â† Lembrei minha senha, voltar ao login
                    </button>
                  </div>
                </form>
              ) : (
                <form onSubmit={handleFinishPasswordReset} className="space-y-4">
                  {simulatedReceivedCode && (
                    <div className="p-3.5 bg-emerald-950/60 border border-emerald-700/60 rounded-2xl space-y-1.5">
                      <div className="flex items-center justify-between text-xs font-bold text-emerald-300">
                        <span>ðŸ”‘ CÃ³digo de RecuperaÃ§Ã£o Gerado:</span>
                        <span className="font-sans text-base text-white tracking-widest">{simulatedReceivedCode}</span>
                      </div>
                      <p className="text-[11px] text-emerald-400/80">
                        CÃ³digo pronto para redefiniÃ§Ã£o imediata de senha no ambiente de testes.
                      </p>
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="000000"
                      className="w-full py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-lg font-sans font-bold text-white text-center tracking-widest focus:border-emerald-500 outline-none"
                    />
                  </div>
                  ...
                  <div className="text-center pt-2">
                    ...
                    >
                      â† Voltar e informar outro e-mail
                    </button>
                  </div>
                </form>
              )}
          ...
          {activeTab === 'register-customer' && (
            <form onSubmit={handleCustomerRegisterSubmit} className="space-y-3.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="JapuÃ­ba">JapuÃ­ba</option>
                    <option value="FaraÃ³">FaraÃ³</option>
                    <option value="GuapiaÃ§u">GuapiaÃ§u</option>
                    <option value="Boa Vista">Boa Vista</option>
                    <option value="ValÃ©rio">ValÃ©rio</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1">
                    Senha de Acesso (min. 6 dÃ­gitos) *
                  </label>
                  <input
                    type="password"
                    required
              ...
              <div className="pt-1">
                <label className="flex items-center space-x-2 text-xs text-slate-400 cursor-pointer">
                  <input
                    ...
                    className="rounded border-slate-700 bg-slate-950 text-emerald-600 focus:ring-emerald-500 w-4 h-4"
                  />
                  <span>Concordo com os Termos de Uso e PolÃ­tica de Privacidade do Achei Aqui</span>
                </label>
              </div>
      ...
      <footer className="py-3 px-4 text-center text-[11px] text-slate-500 border-t border-slate-900 bg-slate-950/80">
        <span>Â© 2026 Achei Aqui â€¢ Cachoeiras de Macacu, RJ â€¢ SeguranÃ§a & AutenticaÃ§Ã£o Verificada</span>
      </footer>
    </div>
  );
};
