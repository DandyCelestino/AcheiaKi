...
export const UserNotificationsList: React.FC = () => {
  ...
  const getCategoryBadge = (cat: NotificationCategory) => {
    switch (cat) {
      ...
      case 'COMISSAO':
        return { icon: DollarSign, label: 'ComissÃ£o/Planos', color: 'bg-amber-100 text-amber-800' };
      case 'SEGURANCA':
        return { icon: ShieldAlert, label: 'SeguranÃ§a', color: 'bg-purple-100 text-purple-800' };
      case 'URGENTE':
        return { icon: AlertTriangle, label: 'Urgente', color: 'bg-red-100 text-red-800' };
      case 'PROMO':
        return { icon: Sparkles, label: 'PromoÃ§Ã£o', color: 'bg-pink-100 text-pink-800' };
      default:
      ...
      <div className="p-4 sm:p-6 bg-gradient-to-r from-slate-900 via-emerald-950 to-slate-900 text-white flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-base sm:text-lg font-black text-white">Minhas NotificaÃ§Ãµes</h3>
            {unreadCount > 0 && (
          ...
          <p className="text-xs text-emerald-100/80 mt-1">
            Receba avisos de pedidos, novidades e comunicados oficiais da administraÃ§Ã£o pelo aplicativo.
          </p>
        </div>
      ...
      <div className="px-4 sm:px-6 pt-2 pb-1 flex flex-wrap items-center justify-between gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input
            ...
            placeholder="Buscar por tÃ­tulo, loja ou cÃ³digo..."
      ...
      <div className="divide-y divide-slate-100 px-4 sm:px-6 pb-6">
        {filteredNotifs.length === 0 ? (
          <div className="py-12 text-center text-slate-400 space-y-2">
            ...
            <p className="text-sm font-bold text-slate-600">Nenhuma notificaÃ§Ã£o localizada</p>
            <p className="text-xs text-slate-400">
              {searchTerm || onlyUnread
                ? 'Nenhuma mensagem corresponde aos filtros selecionados.'
                : 'Todas as suas mensagens da administraÃ§Ã£o e pedidos aparecerÃ£o nesta tela.'}
            </p>
          </div>
        ) : (
