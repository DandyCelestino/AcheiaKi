...
interface CheckoutModalProps {
  ...
  initialModality?: 'DELIVERY' | 'RETIRADA' | 'EXPERIMENTAÃ‡ÃƒO';
...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  const handleProceedToVerification = async (e: React.FormEvent) => {
    ...
    if (!customerPhone.trim() || customerPhone.replace(/\D/g, '').length < 8) {
      triggerToast('Por favor, informe um nÃºmero de telefone/WhatsApp vÃ¡lido.');
      return;
    }
    ...
    if (modality === 'DELIVERY' && !customerAddress.trim()) {
      triggerToast('Por favor, informe o endereÃ§o de entrega em Cachoeiras de Macacu.');
      return;
    }
    if (!termsAccepted) {
    ...
    try {
      ...
      triggerToast(`CÃ³digo de verificaÃ§Ã£o enviado via ${verificationChannel === 'WHATSAPP' ? 'WhatsApp' : 'SMS'}!`);
    } catch (err: any) {
  ...
  const handleResendCode = async (channelOverride?: NotificationChannel) => {
    ...
    try {
      ...
      setResendCooldown(30);
      triggerToast(`CÃ³digo reenviado com sucesso via ${targetChannel === 'WHATSAPP' ? 'WhatsApp' : 'SMS'}!`);
    } catch (err: any) {
      triggerToast(`Erro ao reenviar cÃ³digo: ${err.message}`);
    } finally {
      setIsDispatchingCode(false);
    }
  };
  ...
  const handleConfirmPhoneVerification = (e?: React.FormEvent) => {
    ...
    if (enteredSmsCode.trim() !== generatedSmsCode) {
      setVerifyError('CÃ³digo de seguranÃ§a incorreto. Verifique o cÃ³digo de 6 dÃ­gitos recebido.');
      return;
    }
    ...
    const created = createOrder({
      ...
      customerAddress: modality === 'DELIVERY' ? customerAddress : `Retirada / BalcÃ£o em ${product.merchantName}`,
      ...
      asaasSplitDetails: [
        {
          ...
          description: 'ComissÃ£o Plataforma Achei Aqui (10% MEI)'
        },
        {
          walletId: asaasWalletId,
          fixedValue: repasseLojista,
          description: `Repasse LÃ­quido Lojista (90%) - ${product.merchantName}`
        }
      ],
      type: 'PRODUTO',
      items: [
    ...
    triggerToast(`Cliente Verificado âœ“! SolicitaÃ§Ã£o ${created.orderNumber || created.code} enviada Ã  loja.`);
  };
  ...
  const handleSimulateStoreRejectStock = () => {
    if (!activeOrder) return;
    rejectOrderStock(activeOrder.id, 'Produto esgotado no estoque fÃ­sico.');
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="font-bold text-sm sm:text-base leading-tight">
                ...
                {currentStep === 'PHONE_VERIFY' && 'ValidaÃ§Ã£o de Telefone / SMS'}
                {currentStep === 'AWAITING_STOCK' && 'Aguardando ConfirmaÃ§Ã£o da Loja'}
                {currentStep === 'STOCK_CONFIRMED' && 'Produto Confirmado & CÃ³digo de NegociaÃ§Ã£o'}
                {currentStep === 'OUT_OF_STOCK' && 'Produto IndisponÃ­vel na Loja'}
              </h3>
              <p className="text-slate-400 text-xs mt-0.5">
                {product.merchantName} â€¢ Consulta e Reserva Local
              </p>
            </div>
          </div>
          <button
        ...
        <div className="bg-emerald-50 px-4 py-2.5 border-b border-emerald-100 flex items-center justify-between text-xs text-emerald-950">
          <div className="flex items-center space-x-2">
            ...
            <p className="text-[11px] leading-tight">
              <strong>Sem cobranÃ§a de cartÃ£o no app:</strong> VocÃª reserva o item com validaÃ§Ã£o e negocia o pagamento (PIX, CartÃ£o ou Dinheiro) diretamente com a loja.
            </p>
          </div>
        </div>
          ...
          {currentStep === 'FORM' && (
            <form onSubmit={handleProceedToVerification} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  ...
                  {product.availableModalities.includes('RETIRADA') && (
                    ...
                    >
                      ...
                      <p className="text-[10px] text-slate-500">BalcÃ£o da loja</p>
                    </button>
                  )}
                  ...
                  {product.availableModalities.includes('EXPERIMENTAÃ‡ÃƒO') && (
                    <button
                      type="button"
                      onClick={() => setModality('EXPERIMENTAÃ‡ÃƒO')}
                      className={`p-3 rounded-xl border text-left transition-all ${
                        modality === 'EXPERIMENTAÃ‡ÃƒO'
                ...
                <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 space-y-3 text-xs">
                  ...
                  <div>
                    <label className="block font-bold text-slate-700 mb-1">
                      EndereÃ§o de Entrega (Rua, NÃºmero e Bairro em Cachoeiras) *
                    </label>
                    <input
                      type="text"
                      required
              ...
              {modality === 'RETIRADA' && (
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-950 space-y-1">
                  ...
                  <p className="text-[11px] text-blue-800">
                    EndereÃ§o: <strong>{product.merchantAddress}</strong>
                  </p>
                </div>
              )}
              ...
              <div className="space-y-3 pt-2 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-900 uppercase">Seus Dados de Contato & VerificaÃ§Ã£o:</span>
                  <span className="text-[10px] text-slate-400">Protegidos atÃ© a confirmaÃ§Ã£o</span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">
                      Telefone / WhatsApp (Para CÃ³digo SMS) *
                    </label>
                    <input
                      type="tel"
                      required
              ...
              {/* Forma de Pagamento / LiquidaÃ§Ã£o Desejada */}
              <div className="space-y-2 pt-2 border-t border-slate-100">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-900 uppercase">
                    Forma de Pagamento & LiquidaÃ§Ã£o:
                  </span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  ...
                  >
                    ...
                    <p className="text-[11px] text-slate-500 mt-1.5 leading-tight">
                      QR Code e Copia e Cola com separaÃ§Ã£o automÃ¡tica no Asaas: 10% plataforma MEI e 90% lojista.
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="text-[11px] text-slate-500 mt-1.5 leading-tight">
                      Pagar na retirada ou entrega (dinheiro ou maquininha fÃ­sica de cartÃ£o).
                    </p>
                  </button>
                </div>
              </div>
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                <label className="flex items-start space-x-2 cursor-pointer text-xs">
                  ...
                  <span className="text-slate-700 text-[11px] leading-relaxed">
                    Declaro interesse no produto e concordo que a plataforma atua como <strong>guia de consulta e reserva local</strong>. O pagamento serÃ¡ combinado e efetuado diretamente com o lojista apÃ³s a confirmaÃ§Ã£o de estoque.
                  </span>
                </label>
              </div>
              ...
              >
                <span>AvanÃ§ar para VerificaÃ§Ã£o por SMS/WhatsApp</span>
            ...
            <div className="space-y-4 py-1">
              <div className="text-center space-y-1.5">
                ...
                <h4 className="text-base sm:text-lg font-black text-slate-900">
                  VerificaÃ§Ã£o de NÃºmero por Gateway
                </h4>
                <p className="text-xs text-slate-500 max-w-md mx-auto">
                  Validamos o seu nÃºmero atravÃ©s de gateway de seguranÃ§a para confirmar sua autenticidade antes do envio da reserva Ã  loja.
                </p>
              </div>
              ...
              <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-slate-600">DestinatÃ¡rio:</span>
                ...
                <div className="pt-2 flex items-center justify-between bg-white p-2.5 rounded-lg border border-slate-200">
                  <div className="text-[11px] text-slate-500">
                    CÃ³digo de Teste: <strong className="font-sans text-slate-800">{generatedSmsCode}</strong>
                  </div>
                  <button
                    type="button"
              ...
              <form onSubmit={handleConfirmPhoneVerification} className="space-y-4">
                ...
                <div className="flex items-center justify-between text-xs pt-1">
                  ...
                  >
                    ...
                    <span>
                      {resendCooldown > 0
                        ? `Reenviar cÃ³digo em ${resendCooldown}s`
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  >
                    ...
                    <span>Validar CÃ³digo & Enviar Pedido</span>
                  </button>
                </div>
              </form>
            </div>
            ...
            <div className="space-y-5 py-2">
              <div className="text-center space-y-2">
                ...
                <h4 className="text-lg font-black text-slate-900">
                  ðŸ”” SolicitaÃ§Ã£o de Compra {activeOrder.orderNumber || activeOrder.code}
                </h4>
                <p className="text-xs text-slate-500 max-w-sm mx-auto">
                  A loja <strong>{product.merchantName}</strong> recebeu o seu pedido e estÃ¡ verificando o estoque fÃ­sico.
                </p>
              </div>
              ...
              <div className="p-4 bg-amber-50 border-2 border-dashed border-amber-300 rounded-2xl text-center space-y-2">
                ...
                <p className="text-[11px] text-amber-800">
                  A loja possui 15 minutos para confirmar o produto. Se confirmado, ele ficarÃ¡ <strong>reservado para vocÃª por 30 minutos</strong>.
                </p>
              </div>
              ...
              <div className="p-4 bg-slate-900 text-white rounded-2xl space-y-2">
                <p className="text-[11px] text-slate-400 font-bold uppercase">
                  Painel de DemonstraÃ§Ã£o (Resposta da Loja Parceira):
                </p>
          ...
          {currentStep === 'STOCK_CONFIRMED' && activeOrder && (
            <div className="space-y-5 py-2">
              <div className="text-center space-y-2">
                ...
                <p className="text-xs text-slate-500 max-w-sm mx-auto">
                  A loja <strong>{product.merchantName}</strong> confirmou a disponibilidade e estÃ¡ pronta para concluir a negociaÃ§Ã£o.
                </p>
              </div>
              ...
              <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between text-xs text-emerald-950">
                ...
                <span className="font-sans text-sm font-black text-emerald-800 bg-white px-2.5 py-0.5 rounded-lg border border-emerald-200">
                  â±ï¸ {formatTimer(reservationTimerSeconds)}
                </span>
              </div>

              {/* SELETOR DE MÃ‰TODO DE LIQUIDAÃ‡ÃƒO / PAGAMENTO */}
              <div className="grid grid-cols-2 p-1 bg-slate-100 rounded-2xl border border-slate-200 text-xs font-bold gap-1">
                ...
                >
                  ...
                  <span>CÃ³digo Presencial</span>
                </button>
              </div>

              {/* CONTEÃšDO DA ABA SELECIONADA */}
              {activeConfirmedTab === 'PIX' ? (
                <PixPaymentModule
                  order={activeOrder}
                ...
                <div className="p-5 bg-linear-to-br from-blue-900 to-indigo-950 text-white rounded-2xl shadow-md text-center space-y-3">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-500/30 text-blue-200 rounded-full text-[11px] font-bold uppercase tracking-wider">
                    ...
                    <span>CÃ³digo de SeguranÃ§a & NegociaÃ§Ã£o</span>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center justify-center space-x-2 my-2">
                      ...
                      <button
                        onClick={() => handleCopyText(activeOrder.securityCode || 'K7P4X9', 'CÃ³digo de SeguranÃ§a')}
                  ...
                  <p className="text-[11px] text-blue-200 max-w-sm mx-auto leading-relaxed">
                    Esse cÃ³digo aparece tanto para vocÃª quanto para a loja. <strong>Quando a entrega ou retirada for realizada:</strong> vocÃª informa o cÃ³digo â†’ a loja confirma no painel â†’ o pedido Ã© concluÃ­do.
                  </p>
                </div>
              )}
              ...
              <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1.5">
                ...
                <div className="flex justify-between text-slate-600">
                  ...
                  <span className="font-bold text-emerald-600">Confirmada âœ“</span>
                </div>
              </div>
            ...
            <div className="space-y-5 py-4 text-center">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                  A loja <strong>{product.merchantName}</strong> informou que o produto <strong>{product.name}</strong> estÃ¡ esgotado temporariamente.
                </p>
              </div>

              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-600">
                Nenhum valor foi cobrado e sua solicitaÃ§Ã£o foi encerrada. VocÃª pode procurar produtos semelhantes de outros comerciantes de Cachoeiras de Macacu.
              </div>
