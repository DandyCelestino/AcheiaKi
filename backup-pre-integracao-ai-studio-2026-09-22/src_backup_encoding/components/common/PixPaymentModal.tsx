...
export const PixPaymentModal: React.FC<PixPaymentModalProps> = ({
  ...
  const pixCopyPasteCode = `00020126580014br.gov.bcb.pix0114${OFFICIAL_PIX_INFO.cnpjClean}520400005303986540${amount.toFixed(2).length < 10 ? '0' + amount.toFixed(2).length : amount.toFixed(2).length}${amount.toFixed(2)}5802BR5924BEX SERVICOS E COMERCIOS6021CACHOEIRAS DE MACACU62070503***6304${Math.floor(1000 + Math.random() * 9000).toString(16).toUpperCase()}`;
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-5">
          ...
          <div className="p-3.5 bg-slate-100 border border-slate-200 rounded-xl text-[11px] text-slate-700 space-y-1.5">
            <div className="flex items-center space-x-1.5 font-bold text-slate-900">
              ...
              <span>Regra de LiberaÃ§Ã£o da Modalidade / Recurso:</span>
            </div>
            <p className="leading-relaxed">
              â€¢ <strong>Se o pagamento for confirmado:</strong> o novo plano/banner/comissÃ£o serÃ¡ <strong>liberado imediatamente</strong> e uma notificaÃ§Ã£o de recebimento serÃ¡ transmitida ao Administrador Master da plataforma.
            </p>
            <p className="leading-relaxed text-amber-900">
              â€¢ <strong>Se nÃ£o for pago:</strong> o novo plano permanecerÃ¡ <strong>bloqueado</strong> com status <em>"Aguardando Pagamento PIX"</em> e seu estabelecimento <strong>permanecerÃ¡ ativo na modalidade atual ({currentTierName})</strong>.
            </p>
          </div>
        </div>
          ...
          >
            {paymentStatus === 'PROCESSING' ? (
              ...
              <>
                ...
                <span>JÃ¡ Realizei o Pagamento via PIX</span>
              </>
            )}
          </button>
        </div>
