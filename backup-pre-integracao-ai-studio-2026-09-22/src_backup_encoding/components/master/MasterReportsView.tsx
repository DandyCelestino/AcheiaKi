...
export const MasterReportsView: React.FC<MasterReportsViewProps> = ({ onOpenDossier }) => {
  ...
  const merchantsCount = useMemo(() => {
    return merchants.filter((m) => m.category !== 'ServiÃ§os & Profissionais').length;
  }, [merchants]);
  ...
  const completedOrdersCount = useMemo(() => {
    return filteredOrders.filter((o) => o.status === 'ConcluÃ­do').length;
  }, [filteredOrders]);

  const activeReservationsCount = useMemo(() => {
    return filteredOrders.filter((o) => o.stockConfirmationStatus === 'STOCK_CONFIRMED' && o.status !== 'ConcluÃ­do').length;
  }, [filteredOrders]);
  ...
  // ============================================================================
  // MODO GRÃFICO - AGREGADORES E SÃ‰RIES DE DADOS VISUAIS
  // ============================================================================

  // 1. Cronologia de Vendas e ComissÃµes (Ãrea / Barras Temporais)
  ...
  // 2. Ranking GrÃ¡fico dos Top Lojistas por Faturamento e ComissÃ£o
  ...
  // 3. DistribuiÃ§Ã£o de Pedidos por Modalidade (Delivery vs Retirada vs Provador)
  const modalityDistribution = useMemo(() => {
    ...
    filteredOrders.forEach((o) => {
      ...
      else if (o.modality === 'EXPERIMENTAÃ‡ÃƒO' || o.modality === 'PROVADOR') provador++;
      else delivery++;
    });

    return [
      { name: 'Delivery / Entrega', value: delivery, color: '#3B82F6' },
      { name: 'Retirada no BalcÃ£o', value: retirada, color: '#10B981' },
      { name: 'Provador / Exp.', value: provador, color: '#F59E0B' }
    ].filter((item) => item.value > 0);
  }, [filteredOrders]);

  // 4. DistribuiÃ§Ã£o de Pedidos por Status Operacional
  const orderStatusDistribution = useMemo(() => {
    ...
    filteredOrders.forEach((o) => {
      if (o.status === 'ConcluÃ­do') concluidos++;
    ...
    return [
      { name: 'ConcluÃ­dos', value: concluidos, color: '#10B981' },
  ...
  // 5. DistribuiÃ§Ã£o de Faturamento por Categoria Comercial
  const categoryDistribution = useMemo(() => {
    const counts: Record<string, number> = {};
    const categoryColors: Record<string, string> = {
      'Gastronomia & Restaurantes': '#EF4444',
      'Moda & VestuÃ¡rio': '#EC4899',
      'Supermercados & Mercearias': '#10B981',
      'FarmÃ¡cias & SaÃºde': '#06B6D4',
      'ServiÃ§os & Profissionais': '#8B5CF6',
      'Pet Shop & Agro': '#F59E0B',
      'Outros ComÃ©rcios': '#64748B'
    };

    filteredOrders.forEach((o) => {
      const m = merchants.find((m) => m.id === o.merchantId);
      const cat = m?.category || 'Outros ComÃ©rcios';
  ...
  const handleExportCSV = () => {
    ...
    triggerToast('RelatÃ³rio CSV exportado com sucesso!');
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-base sm:text-lg font-black text-slate-900">
              RelatÃ³rios DiÃ¡rios & MovimentaÃ§Ãµes em Tempo Real
            </h2>
          </div>
          <p className="text-xs text-slate-500 mt-0.5">
            Acompanhamento consolidado de vendas, comissÃµes arrecadadas, reservas e movimentaÃ§Ãµes no comÃ©rcio de Cachoeiras de Macacu.
          </p>
        </div>

        <div className="flex items-center space-x-2 flex-wrap gap-y-2">
          {/* MODO GRÃFICO vs MODO TABELA TOGGLE */}
        ...
        <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-2xs flex items-center justify-between">
          <div className="space-y-1">
            ...
            <span className="text-[10px] text-purple-700 font-semibold flex items-center gap-1">
              ...
              {services.length} ServiÃ§os Ativos
            </span>
          </div>
      ...
      {/* FINANCIAL METRICS - GMV, COMISSÃ•ES, REPASSE E RESERVAS */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-linear-to-br from-slate-900 to-slate-950 text-white rounded-2xl shadow-sm space-y-2">
          ...
          <span className="text-[10px] text-blue-300 block">
            {filteredOrders.length} pedidos no perÃ­odo
          </span>
        </div>

        {/* ComissÃµes do Master */}
        ...
        <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-2xs space-y-2">
          ...
          <span className="text-[10px] text-slate-500 block">
            Faturamento lÃ­quido dos parceiros
          </span>
        </div>

        {/* Reservas & ConclusÃµes */}
        <div className="p-4 bg-white border border-slate-200 rounded-2xl shadow-2xs space-y-2">
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
            Pedidos ConcluÃ­dos & Reservas
          </span>
          <div className="text-2xl font-black text-slate-900">
            {completedOrdersCount} <span className="text-xs font-normal text-slate-400">concluÃ­dos</span>
          </div>
      ...
      {/* ========================================================================= */}
      {/* SEÃ‡ÃƒO 1: MODO GRÃFICO (PAINEL ANALÃTICO VISUAL EM TEMPO REAL) */}
      {/* ========================================================================= */}
      {(viewMode === 'grafico' || viewMode === 'ambos') && (
        <div id="master-reports-modo-grafico" className="space-y-6">
          {/* HEADER DO MODO GRÃFICO */}
          <div className="bg-linear-to-r from-slate-900 via-blue-950 to-slate-900 text-white p-5 rounded-2xl shadow-sm border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-base font-black text-white">
                    Modo GrÃ¡fico Ativo: MÃ©tricas Visuais & Desempenho
                  </h3>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  AnÃ¡lise grÃ¡fica do faturamento bruto (GMV), comissÃµes Master, modalidades de entrega e ranking de estabelecimentos parceiros.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs">
              <span className="bg-slate-800 text-slate-300 px-3 py-1.5 rounded-xl border border-slate-700 font-medium">
                {filteredOrders.length} transaÃ§Ãµes analisadas
              </span>
            </div>
          </div>

          {/* GRID DE GRÃFICOS PRIMÃRIOS */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* GRÃFICO 1: EVOLUÃ‡ÃƒO TEMPORAL DE VENDAS & COMISSÃ•ES */}
            <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs flex flex-col justify-between">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h4 className="text-sm font-black text-slate-900 flex items-center gap-2">
                    ...
                    Cronologia de Faturamento: GMV vs. ComissÃµes
                  </h4>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Comparativo entre volume transacionado bruto, repasse aos lojistas e receita da plataforma no perÃ­odo selecionado.
                  </p>
                </div>
                ...
                <div className="w-full">
                  <SvgBarChart
                    ...
                    series={[
                      ...
                      { key: 'liquido', name: 'Repasse Lojista (LÃ­quido)', color: '#059669' },
                      { key: 'comissao', name: 'ComissÃ£o Master', color: '#7C3AED' }
                    ]}
                    height={240}
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between text-xs text-slate-500 gap-2">
                ...
                <span className="flex items-center gap-1.5 font-medium">
                  ...
                  ComissÃ£o Master: <strong>{totalCommissions.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</strong>
                </span>
              </div>
            </div>

            {/* GRÃFICO 2: DISTRIBUIÃ‡ÃƒO POR MODALIDADE (DELIVERY vs RETIRADA) */}
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  ProporÃ§Ã£o de pedidos por entrega em domicÃ­lio vs retirada balcÃ£o.
                </p>
              </div>
          ...
          {/* GRID DE GRÃFICOS SECUNDÃRIOS */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* GRÃFICO 3: RANKING VISUAL DOS TOP LOJISTAS */}
            <div className="lg:col-span-2 bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs flex flex-col justify-between">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h4 className="text-sm font-black text-slate-900 flex items-center gap-2">
                    <Store className="w-4 h-4 text-purple-600" />
                    Top Estabelecimentos: Faturamento & ComissÃ£o Master
                  </h4>
                  <p className="text-xs text-slate-500 mt-0.5">
              ...
              {topMerchantsChartData.length === 0 ? (
                <div className="h-56 flex items-center justify-center text-xs text-slate-400">
                  Nenhum faturamento registrado para o ranking no perÃ­odo.
                </div>
              ) : (
                <div className="w-full">
                  <SvgBarChart
                    ...
                    series={[
                      ...
                      { key: 'commission', name: 'ComissÃ£o Master (R$)', color: '#8B5CF6' }
                    ]}
                    height={220}
                    yFormatter={(val) => `R$ ${val >= 1000 ? `${(val / 1000).toFixed(1)}k` : val}`}
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                <span>Taxa mÃ©dia praticada: <strong>10%</strong></span>
                <button
                  onClick={() => setViewMode('tabela')}
            ...
            {/* GRÃFICO 4: DISTRIBUIÃ‡ÃƒO POR CATEGORIA COMERCIAL */}
            <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs flex flex-col justify-between">
              <div>
                <h4 className="text-sm font-black text-slate-900 flex items-center gap-2">
                  <PieChartIcon className="w-4 h-4 text-amber-600" />
                  Segmentos do ComÃ©rcio Local
                </h4>
                <p className="text-xs text-slate-500 mt-0.5">
                  Volume de compras distribuÃ­do por setor de atividade.
                </p>
              </div>
              ...
              <div className="py-4">
                {categoryDistribution.length === 0 ? (
                  <div className="h-44 flex items-center justify-center text-xs text-slate-400">
                    Sem transaÃ§Ãµes por categoria registradas.
                  </div>
                ) : (
                  <SvgPieChart
              ...
              <div className="pt-3 border-t border-slate-100 text-[11px] text-slate-500 text-center font-medium">
                Gastronomia, Moda e FarmÃ¡cia lideram o fluxo comercial
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ========================================================================= */}
      {/* SEÃ‡ÃƒO 2: MODO TABELA (EXTRATO COMPLETO DE VENDAS & AUDITORIA) */}
      {/* ========================================================================= */}
        ...
        <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-black text-slate-900">
              Extrato Completo de Vendas & ComissÃµes (Modo Tabela)
            </h3>
            <p className="text-xs text-slate-500">
              Clique na foto ou nome do cliente/lojista para inspecionar o DossiÃª 360Â°.
            </p>
          </div>
          ...
          <div className="flex items-center space-x-2">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Buscar por cÃ³digo, cliente ou loja..."
                value={searchQuery}
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-4 py-3">ComissÃ£o Master</th>
                  <th className="px-4 py-3">LÃ­quido Loja</th>
                  <th className="px-4 py-3">Modalidade</th>
                  <th className="px-4 py-3">Status</th>
                  <th className="px-4 py-3 text-right">DossiÃª</th>
                </tr>
              </thead>
                      ...
                      <td className="px-4 py-3">
                        ...
                        {order.securityCode && (
                          <span className="font-sans text-[10px] text-purple-700 font-bold">
                            CÃ³d: {order.securityCode}
                          </span>
                        )}
                      </td>

                      {/* Cliente com foto clicÃ¡vel */}
                      <td className="px-4 py-3">
                        <button
                          type="button"
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-slate-400">{merchant?.category || 'ComÃ©rcio'}</span>
                          </div>
                        </button>
                      </td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
          <h3 className="text-sm font-black text-slate-900 flex items-center gap-2">
            <TrendingUp className="w-4 h-4 text-blue-600" />
            <span>Faturamento & ComissÃµes por Estabelecimento</span>
          </h3>

          {merchantPerformance.length === 0 ? (
            <div className="p-8 text-center text-slate-400 text-xs">
              Nenhuma venda computada para os estabelecimentos no perÃ­odo.
            </div>
          ) : (
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-slate-500">
                        {item.orderCount} vendas â€¢ Taxa: {item.merchant.commissionRate ?? 10}%
                      </span>
                    </div>
                  </button>
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[10px] text-purple-700 font-bold">
                      ComissÃ£o: {item.commission.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                    </span>
                  </div>
                </div>
              ))}
        ...
        {/* TransmissÃ£o de ManifestaÃ§Ãµes & Eventos ao Vivo */}
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black text-slate-900 flex items-center gap-2">
              <Activity className="w-4 h-4 text-emerald-600" />
              <span>ManifestaÃ§Ãµes & Eventos em Tempo Real</span>
            </h3>
            ...
            {recentEvents.map((log, idx) => (
              ...
              >
                ...
                <div className="flex items-center space-x-2 text-[9px] text-slate-400 font-sans">
                  <span>UsuÃ¡rio: {log.userEmail}</span>
                  <span>â€¢</span>
