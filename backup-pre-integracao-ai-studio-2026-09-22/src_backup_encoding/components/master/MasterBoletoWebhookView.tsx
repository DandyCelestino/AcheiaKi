...
export const MasterBoletoWebhookView: React.FC = () => {
  ...
  const handlePurgeAndGoLive = async () => {
    if (!window.confirm('Confirma a inicializaÃ§Ã£o oficial de produÃ§Ã£o? Isso removerÃ¡ entidades de teste e zerarÃ¡ os logs de webhook.')) {
      return;
    }
    setIsPurging(true);
    try {
      ...
      await resp.json();
      clearWebhookLogs();
      triggerToast('Ambiente de produÃ§Ã£o inicializado com sucesso! Logs e testes zerados.', 'success');
    } catch (e: any) {
      clearWebhookLogs();
  ...
  const copyToClipboard = (text: string, keyName: string) => {
    ...
    triggerToast('Copiado para a Ã¡rea de transferÃªncia!');
  ...
  const handleRunSimulation = () => {
    try {
      ...
      setIsSimulating(false);
      triggerToast('Erro na simulaÃ§Ã£o: JSON de payload invÃ¡lido.');
    }
  };
  ...
  const handleReprocess = (event: BoletoWebhookEvent) => {
    ...
    if (!targetCode) {
      triggerToast('Informe o cÃ³digo do boleto para vincular e reprocessar.');
      return;
    }
    ...
    if (success) {
      ...
      triggerToast('Evento reprocessado com sucesso! ComissÃ£o liberada.');
    } else {
      triggerToast('Falha ao reprocessar: cÃ³digo de boleto nÃ£o encontrado.');
    }
  };
  ...
  const getGatewayBadge = (gateway: WebhookGateway) => {
    switch (gateway) {
      ...
      case 'GERENCIANET_EFI':
        return <span className="px-2 py-0.5 rounded text-xs font-bold bg-purple-100 text-purple-800 dark:bg-purple-900/40 dark:text-purple-300">EfÃ­ / Gerencianet</span>;
      default:
      ...
      case 'UNMATCHED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">
            <AlertTriangle className="w-3.5 h-3.5" /> Boleto NÃ£o Encontrado
          </span>
        );
      case 'ERROR':
        return (
    ...
    <div className="space-y-6" id="master-boleto-webhook-module">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-medium mb-3 border border-emerald-500/30">
              ...
              Webhook Engine v2.4 â€¢ AutomaÃ§Ã£o Financeira Ativa
            </div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-white flex items-center gap-3">
              <Webhook className="w-8 h-8 text-emerald-400" />
              Camada de Webhooks de Boletos & ComissÃµes
            </h1>
            <p className="text-emerald-100/80 text-sm mt-1 max-w-3xl">
              Captura em tempo real os avisos de quitaÃ§Ã£o emitidos pelos gateways bancÃ¡rios brasileiros (Asaas, Mercado Pago, Banco Inter, Iugu e EfÃ­),
              atualizando instantaneamente o status da cobranÃ§a e liberando a comissÃ£o do consultor comercial.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              title="Limpar entidades de testes, zerar logs de webhook e inicializar produÃ§Ã£o oficial"
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="text-xs text-slate-500 dark:text-slate-400 mt-2">
            Liberadas no ato da compensaÃ§Ã£o
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="text-xs text-amber-600 dark:text-amber-400 mt-2 font-medium">
            {stats.unmatchedCount === 0 ? 'Tudo conciliado' : 'Requer conciliaÃ§Ã£o manual'}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="text-xs text-emerald-600 dark:text-emerald-400 mt-2 font-medium">
            ExecuÃ§Ã£o assÃ­ncrona instantÃ¢nea
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col md:flex-row md:items-center justify-between gap-3 bg-slate-50/50 dark:bg-slate-900/30">
            <div className="flex flex-1 items-center gap-3">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  placeholder="Buscar por cÃ³digo (ex: BOL-2026-001), cliente, vendedor ou TxID..."
              ...
              >
                ...
                <option value="UNMATCHED">NÃ£o Identificados</option>
              ...
              >
                ...
                <option value="GERENCIANET_EFI">EfÃ­ / Gerencianet</option>
                <option value="GENERIC">API GenÃ©rica</option>
              </select>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  if (confirm('Deseja realmente limpar todo o histÃ³rico de logs de webhook?')) {
                    clearWebhookLogs();
                  }
                }}
            ...
            <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
              <thead className="bg-slate-100/70 dark:bg-slate-900/50 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                <tr>
                  ...
                  <th className="px-4 py-3">Consultor & ComissÃ£o</th>
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5">
                        <div className="font-medium text-slate-900 dark:text-white truncate max-w-[180px]">
                          {evt.clientName || 'NÃ£o identificado'}
                        </div>
                      ...
                      <td className="px-4 py-3.5">
                        <span className="font-bold text-slate-900 dark:text-white">
                          {evt.amountPaid ? `R$ ${evt.amountPaid.toFixed(2)}` : 'â€”'}
                        </span>
                      </td>
      ...
      {activeTab === 'asaas-orders' && (
        <div className="space-y-6">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white">
                    Processamento AutomÃ¡tico de Pagamentos Asaas
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-2xl">
                    O endpoint escuta notificaÃ§Ãµes HTTP POST do Asaas em tempo real. Quando o cliente paga via Pix ou cartÃ£o, o status do pedido Ã© atualizado automaticamente para <strong>Confirmado (PAGO)</strong>. Caso o prazo expire, o pedido passa automaticamente para <strong>Cancelado (EXPIRADO)</strong>.
                  </p>
                </div>
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 rounded-xl border border-slate-700 shadow-sm flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-300 leading-relaxed mb-3">
                  Selecione um pedido e dispare uma notificaÃ§Ã£o idÃªntica Ã  que o Asaas envia para testar a conciliaÃ§Ã£o automÃ¡tica imediata.
                </p>
                ...
                >
                  ...
                  {orders.map((o) => (
                    <option key={o.id} value={o.id}>
                      {o.orderNumber || o.code} - {o.customerName} (R$ {o.totalAmount.toFixed(2)}) â€¢ {o.status} / {o.paymentStatus || 'PENDENTE'}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2">
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      triggerToast(`Webhook Asaas Confirmado: Pedido ${targetOrd.orderNumber || targetOrd.code} agora estÃ¡ PAGO!`);
                    } catch (e: any) {
                ...
                >
                  <CheckCircle2 className="w-3.5 h-3.5" />
                  {isTestingAsaas ? 'Processando...' : 'Simular ConfirmaÃ§Ã£o'}
                </button>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setAsaasTestResult(res);
                      triggerToast(`Webhook Asaas Expirado: Pedido ${targetOrd.orderNumber || targetOrd.code} agora estÃ¡ EXPIRADO!`);
                    } catch (e: any) {
                      triggerToast(`Erro no teste: ${e.message}`);
                    } finally {
                      setIsTestingAsaas(false);
                ...
                >
                  ...
                  {isTestingAsaas ? 'Processando...' : 'Simular ExpiraÃ§Ã£o'}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="bg-slate-900 text-white p-4 rounded-xl border border-slate-800 shadow-md">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-emerald-400 flex items-center gap-1">
                  ...
                  Ãšltima Resposta do Webhook Asaas
                </span>
          ...
          {/* Tabela de Pedidos e ConciliaÃ§Ã£o Asaas */}
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/30">
              <div>
                <h4 className="font-bold text-slate-900 dark:text-white text-sm">
                  Base de Pedidos & ConciliaÃ§Ã£o em Tempo Real ({orders.length})
                </h4>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Os status refletem a base de dados sincronizada automaticamente pelas notificaÃ§Ãµes do webhook.
                </p>
              </div>
              <button
                onClick={() => {
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-100/50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold">
                    <th className="px-4 py-3">CÃ³digo / Pedido</th>
                    ...
                    <th className="px-4 py-3">AÃ§Ãµes de Teste</th>
                  </tr>
                </thead>
                      ...
                      <tr key={ord.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors">
                        ...
                        <td className="px-4 py-3 space-x-1.5 whitespace-nowrap">
                          <button
                            ...
                            title="Disparar confirmaÃ§Ã£o Asaas"
                          >
                            Confirmar
                          </button>
                          <button
                            ...
                            title="Disparar expiraÃ§Ã£o Asaas"
                          >
                            Expirar
                          </button>
                        </td>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  >
                    ...
                    <option value="BANCO_INTER">Banco Inter (CobranÃ§a v2 - PAGO)</option>
                    <option value="IUGU">Iugu (invoice.status_changed - paid)</option>
                    <option value="GERENCIANET_EFI">EfÃ­ / Gerencianet (NotificaÃ§Ã£o de Pagamento)</option>
                    <option value="GENERIC">Achei Aqui Direct API (JSON PadrÃ£o)</option>
                  </select>
                  ...
                  >
                    ...
                    {boletoRequests.map((b) => (
                      <option key={b.id} value={b.id}>
                        {b.code} - {b.clientName} (R$ {b.amount.toFixed(2)}) â€¢ Status: {b.status}
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">
                    CÃ³digo do Boleto / ReferÃªncia (ForÃ§ar no Payload)
                  </label>
                  <input
                    type="text"
                    value={simCustomCode}
                  ...
                  <span className="text-[11px] text-slate-400 mt-1 block">
                    Altere para testar o comportamento com cÃ³digos nÃ£o existentes (status UNMATCHED).
                  </span>
                </div>
                ...
                <div className="pt-2">
                  ...
                  >
                    {isSimulating ? (
                      ...
                      <>
                        <Play className="w-4 h-4 fill-current" />
                        Disparar Webhook de SimulaÃ§Ã£o
                      </>
                    )}
                  </button>
                </div>
                  ...
                  >
                    ...
                    {simulationResult.event && (
                      <div className="mt-2 text-[11px] space-y-1">
                        ...
                        <div>
                          <strong>Tempo de ExecuÃ§Ã£o:</strong> {simulationResult.event.durationMs} ms
                        </div>
                        {simulationResult.event.commissionReleased && (
                          <div className="text-emerald-700 dark:text-emerald-300 font-semibold">
                            âœ“ ComissÃ£o de R$ {simulationResult.event.commissionAmount?.toFixed(2)} liberada com sucesso!
                          </div>
                        )}
                      </div>
                    )}
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="bg-white dark:bg-slate-800 p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col h-full">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-slate-400">EditÃ¡vel em tempo real</span>
              </div>
              <textarea
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              ConfiguraÃ§Ãµes da Camada de Webhooks & AutomaÃ§Ã£o Financeira
            </h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Defina as regras automÃ¡ticas de liquidaÃ§Ã£o, credenciais de assinatura e gatilhos disparados na recepÃ§Ã£o de confirmaÃ§Ãµes.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-700">
            ...
            <div className="space-y-4">
              <h4 className="font-semibold text-slate-900 dark:text-white text-sm flex items-center gap-2">
                ...
                SeguranÃ§a e Endpoint de Ingress
              </h4>
              ...
              <div>
                ...
                <span className="text-[11px] text-slate-400 mt-1 block">
                  Utilizada na verificaÃ§Ã£o do cabeÃ§alho HMAC / X-Signature dos gateways.
                </span>
              </div>
            </div>
              ...
              <div className="space-y-3">
                <label className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer">
                  ...
                  <div>
                    <span className="text-sm font-semibold text-slate-900 dark:text-white block">
                      Liberar ComissÃ£o do Vendedor Imediatamente
                    </span>
                    <span className="text-xs text-slate-500 dark:text-slate-400">
                      Muda o status da comissÃ£o para "LIBERADA" assim que a compensaÃ§Ã£o bancÃ¡ria for confirmada pelo webhook.
                    </span>
                  </div>
                </label>
                ...
                <label className="flex items-start gap-3 p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400">
                      Dispara uma notificaÃ§Ã£o in-app em tempo real para o consultor celebrando a liberaÃ§Ã£o da comissÃ£o.
                    </span>
                  </div>
                </label>
              </div>
      ...
      {/* VIEW: DOCUMENTAÃ‡ÃƒO & INSTRUÃ‡Ã•ES DE INTEGRAÃ‡ÃƒO */}
      {activeTab === 'docs' && (
        <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6">
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <Code className="w-5 h-5 text-emerald-500" />
              Guia PrÃ¡tico de IntegraÃ§Ã£o de Webhooks para Boletos
            </h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Configure a URL de notificaÃ§Ã£o diretamente no painel do seu banco ou gateway de cobranÃ§a para ativaÃ§Ã£o em produÃ§Ã£o.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 space-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                No painel Asaas, acesse <strong>ConfiguraÃ§Ãµes da Conta &gt; IntegraÃ§Ãµes &gt; Webhooks &gt; CobranÃ§as</strong>.
                Ative o evento <code>CobranÃ§a Recebida (PAYMENT_RECEIVED)</code> e aponte para a URL do endpoint Achei Aqui.
              </p>
            ...
            <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 space-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                ...
                O campo <code>external_reference</code> conterÃ¡ o cÃ³digo do boleto emitido.
              </p>
            ...
            <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-orange-100 text-orange-800">API CobranÃ§a v2</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Cadastre o webhook via endpoint <code>PUT /cobranca/v2/boletos/webhook</code> do Banco Inter.
                O campo <code>seuNumero</code> mapearÃ¡ o cÃ³digo do boleto no Achei Aqui.
              </p>
            ...
            <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 space-y-2">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                ...
                O campo <code>order_id</code> conterÃ¡ o cÃ³digo do boleto.
              </p>
            ...
            <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 space-y-2">
              <div className="flex items-center justify-between">
                <h4 className="font-bold text-slate-900 dark:text-white text-sm">5. EfÃ­ Bank</h4>
                <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-purple-100 text-purple-800">Gerencianet</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Na EfÃ­, ative as notificaÃ§Ãµes de alteraÃ§Ã£o de status de cobranÃ§a informando o parÃ¢metro <code>custom_id</code>
            ...
            <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/30 space-y-2">
              <div className="flex items-center justify-between">
                <h4 className="font-bold text-slate-900 dark:text-white text-sm">6. API PrÃ³pria</h4>
                <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-slate-100 text-slate-800">JSON Direto</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                Qualquer sistema de automaÃ§Ã£o ou ERP externo pode postar diretamente um JSON no endpoint informado com
            ...
            <div className="grid grid-cols-2 gap-3 text-xs bg-slate-50 dark:bg-slate-800/50 p-3.5 rounded-xl">
              ...
              <div>
                ...
                <span className="font-sans font-bold text-slate-800 dark:text-slate-200">
                  {selectedEvent.boletoCode || 'NÃ£o identificado'}
                </span>
              </div>
              <div>
                <span className="text-slate-400 block">Consultor Comercial:</span>
                <span className="font-medium text-slate-800 dark:text-slate-200">
                  {selectedEvent.agentName || 'â€”'}
                </span>
              </div>
              <div>
                <span className="text-slate-400 block">ComissÃ£o Liberada:</span>
                <span className="font-bold text-emerald-600 dark:text-emerald-400">
                  {selectedEvent.commissionAmount ? `R$ ${selectedEvent.commissionAmount.toFixed(2)}` : 'â€”'}
                </span>
              </div>
              <div>
                <span className="text-slate-400 block">TxID / Gateway ID:</span>
                <span className="font-sans text-slate-700 dark:text-slate-300">
                  {selectedEvent.externalTransactionId || 'â€”'}
                </span>
              </div>
              <div>
                <span className="text-slate-400 block">Data / DuraÃ§Ã£o:</span>
            ...
            {selectedEvent.status === 'UNMATCHED' && (
              <div className="p-3.5 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl space-y-2">
                ...
                <p className="text-xs text-amber-700 dark:text-amber-300">
                  Informe o cÃ³digo do boleto correto para liquidar e liberar a comissÃ£o:
                </p>
