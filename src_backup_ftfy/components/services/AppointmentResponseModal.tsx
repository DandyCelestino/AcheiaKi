...
export const AppointmentResponseModal: React.FC<AppointmentResponseModalProps> = ({
  ...
  const [responseMessage, setResponseMessage] = useState(
    order?.serviceDetails?.merchantResponse?.responseMessage ||
      `OlÃ¡ ${order?.customerName}! Seu horÃ¡rio foi confirmado com sucesso. Estaremos prontos para atendÃª-lo(a).`
  );
  const [instructions, setInstructions] = useState(
    order?.serviceDetails?.merchantResponse?.instructionsForCustomer ||
      'Por favor, chegue com 10 minutos de antecedÃªncia. Em caso de dÃºvidas, nos chame no WhatsApp.'
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (responseStatus === 'CONCLUIDO') nextOrderStatus = 'ConcluÃ­do';
  ...
  const handleWhatsAppSend = () => {
    const text = encodeURIComponent(
      `*Achei Aqui - ConfirmaÃ§Ã£o de Agendamento (${order.code})*\n` +
      `OlÃ¡ *${order.customerName}*!\n\n` +
      `*Estabelecimento:* ${order.merchantName}\n` +
      `*ServiÃ§o:* ${order.serviceDetails?.serviceTitle}\n` +
      `*Status:* ${responseStatus}\n` +
      `*Data/Hora:* ${responseStatus === 'REAGENDADO' ? proposedDate : order.serviceDetails?.scheduledDate} Ã s ${responseStatus === 'REAGENDADO' ? proposedTime : order.serviceDetails?.scheduledTime}\n` +
      `*Valor:* R$ ${(parseFloat(confirmedPrice) || order.totalAmount).toFixed(2).replace('.', ',')}\n\n` +
      `*Mensagem do Prestador:* ${responseMessage}\n` +
      (instructions ? `*OrientaÃ§Ãµes:* ${instructions}\n` : '') +
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs">
                Pedido {order.code} â€¢ {order.customerName}
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-4 text-slate-800">
          ...
          <div className="p-3.5 bg-blue-50/70 border border-blue-200 rounded-xl space-y-1.5 text-xs">
            <div className="flex justify-between font-bold text-blue-950">
              <span>{order.serviceDetails?.serviceTitle || 'ServiÃ§o Solicitado'}</span>
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-slate-700">
              ...
              <span>Data Solicitada: <strong>{order.serviceDetails?.scheduledDate} Ã s {order.serviceDetails?.scheduledTime}</strong></span>
            ...
            {order.serviceDetails?.customerNotes && (
              <div className="pt-1.5 border-t border-blue-100 text-blue-900 italic">
                <strong>ObservaÃ§Ãµes do Cliente:</strong> "{order.serviceDetails.customerNotes}"
              </div>
            )}
          </div>
          ...
          {responseStatus === 'REAGENDADO' && (
            <div className="p-3.5 bg-amber-50 rounded-xl border border-amber-200 space-y-3">
              <span className="text-xs font-bold text-amber-950 block">
                Novo HorÃ¡rio Sugerido para o Cliente:
              </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">
                Nome do ResponsÃ¡vel pela Resposta:
              </label>
              <input
                type="text"
                value={authorName}
                onChange={(e) => setAuthorName(e.target.value)}
                placeholder="Ex: Dr. MaurÃ­cio / Prof. Carlos"
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Escreva a confirmaÃ§Ã£o ou proposta de reagendamento..."
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">
              OrientaÃ§Ãµes de Preparo / Documentos / Chegada (Opcional):
            </label>
            <textarea
              rows={2}
              value={instructions}
              onChange={(e) => setInstructions(e.target.value)}
              placeholder="Ex: Chegar com 10 min de antecedÃªncia, trazer radiografias anteriores ou documento com foto..."
              className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs outline-none focus:bg-white focus:border-blue-500 leading-relaxed"
            />
          </div>
          ...
          <div className="pt-2 space-y-2">
            ...
            {order.customerPhone && (
              ...
              >
                ...
                <span>Enviar ConfirmaÃ§Ã£o tambÃ©m pelo WhatsApp</span>
              </button>
            )}
          </div>
        </form>
