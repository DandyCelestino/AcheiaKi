...
export const CustomerAccountView: React.FC<CustomerAccountViewProps> = ({
    ...
    return (
      <div className="max-w-md mx-auto my-12 p-8 bg-white rounded-2xl border border-slate-200 text-center space-y-4 shadow-sm">
        ...
        <p className="text-xs text-slate-500">
          FaÃ§a login para acompanhar seus pedidos, cÃ³digos de retirada e agendamentos no comÃ©rcio local.
        </p>
        <button
          onClick={onOpenAuth}
  ...
  const trialOrders = userOrders.filter((o) => o.modality === 'EXPERIMENTAÃ‡ÃƒO');
  ...
  const copyCode = (code: string) => {
    if (navigator.clipboard) {
      ...
      triggerToast(`CÃ³digo ${code} copiado!`);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      {/* Atalho de Acesso RÃ¡pido ao Painel Pessoal do Vendedor */}
        ...
        <div className="p-4 bg-gradient-to-r from-indigo-950 via-blue-900 to-slate-900 text-white rounded-2xl border border-indigo-700/60 shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <h4 className="text-sm font-black text-white">VocÃª estÃ¡ conectado como Vendedor Comercial</h4>
              <p className="text-xs text-indigo-200/90 mt-0.5">
                Seu acesso ao Portal do Vendedor estÃ¡ liberado com suas comissÃµes, boletos e metas.
              </p>
            </div>
          </div>
          <button
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">{currentUser.email} â€¢ {currentUser.phone}</p>
      ...
      <div className="flex overflow-x-auto space-x-2 pb-2 scrollbar-none text-xs font-bold border-b border-slate-200">
        ...
        >
          ...
          <span>PreferÃªncias de NotificaÃ§Ã£o</span>
          {unreadNotifCount > 0 && (
        ...
        >
          ...
          <span>CÃ³digos de Retirada ({pickupOrders.length})</span>
        </button>
        ...
        >
          ...
          <span>Minha ReputaÃ§Ã£o & AvaliaÃ§Ãµes</span>
        </button>
      </div>
      ...
      {/* TAB CONTENT: PREFERÃŠNCIAS DE NOTIFICAÃ‡ÃƒO */}
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-slate-400 text-xs">â€¢ {order.createdAt}</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-xs">
                  ...
                  <div className="space-y-1">
                    ...
                    {order.stockConfirmationStatus === 'STOCK_CONFIRMED' ? (
                      <div className="text-emerald-800 font-bold flex items-center gap-1">
                        ...
                        <span>Estoque Confirmado e Separado âœ“</span>
                      </div>
                      ...
                      <div className="text-amber-800 font-bold flex items-center gap-1">
                        ...
                        <span>Em AnÃ¡lise pelo Lojista</span>
                      </div>
                    )}
                  ...
                  {/* Column 2: InformaÃ§Ãµes de Pagamento */}
                  <div className="space-y-1 border-t md:border-t-0 md:border-l border-slate-200 pt-2 md:pt-0 md:pl-3">
                    ...
                    <span className="text-[11px] text-slate-600 block">
                      Modalidade: {order.paymentMethod || 'Acordo Direto com a Loja (PIX / Dinheiro / CartÃ£o)'}
                    </span>
                  </div>

                  {/* Column 3: InformaÃ§Ãµes de Envio / Retirada */}
                  <div className="space-y-1 border-t md:border-t-0 md:border-l border-slate-200 pt-2 md:pt-0 md:pl-3">
                    <span className="text-[10px] font-bold uppercase text-slate-500 block">
                      3. Envio / Retirada & ConclusÃ£o:
                    </span>
                ...
                {order.serviceDetails && (
                  <div className="p-3 bg-blue-50/70 rounded-xl text-xs space-y-1">
                    <p className="font-bold text-blue-950">
                      ðŸ’† {order.serviceDetails.serviceTitle}
                    </p>
                    <p className="text-blue-800">
                      Profissional: {order.serviceDetails.professional} â€¢ Data: {order.serviceDetails.scheduledDate} Ã s {order.serviceDetails.scheduledTime}
                    </p>
                  </div>
                )}
                ...
                {order.stockConfirmationStatus === 'PENDING_STORE_CONFIRMATION' && (
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between text-xs text-amber-900">
                    <div className="flex items-center space-x-2">
                      ...
                      <span><strong>Aguardando confirmaÃ§Ã£o de estoque:</strong> A loja possui atÃ© 15 minutos para verificar a disponibilidade.</span>
                    </div>
                  </div>
                )}
                ...
                {order.stockConfirmationStatus === 'STOCK_CONFIRMED' && (
                  <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center justify-between text-xs text-emerald-950">
                    <div className="flex items-center space-x-2">
                      ...
                      <span><strong>Estoque Confirmado pela Loja!</strong> Produto em reserva temporÃ¡ria de 30 minutos.</span>
                    </div>
                  </div>
                )}
                  ...
                  <div className="p-3.5 bg-linear-to-r from-blue-900 to-indigo-900 text-white rounded-xl flex items-center justify-between shadow-xs">
                    <div>
                      <span className="text-[10px] text-blue-300 font-bold uppercase tracking-wider block">
                        CÃ³digo Ãšnico de SeguranÃ§a & NegociaÃ§Ã£o:
                      </span>
                  ...
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-between">
                    <div>
                      <span className="text-[10px] text-blue-700 font-bold uppercase">
                        CÃ³digo de Retirada no BalcÃ£o:
                      </span>
                    ...
                    <div className="p-4 bg-gradient-to-r from-emerald-950 via-slate-900 to-teal-950 text-white rounded-2xl shadow-md space-y-3">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <div>
                            <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider block">
                              Achei Aqui Delivery â€¢ Rastreamento
                            </span>
                      ...
                      <div className="text-xs space-y-1">
                        <p className="text-slate-300">
                          ...
                          {linkedRide.status === 'ACEITA' && `Entregador ${linkedRide.driverName} aceitou a corrida e estÃ¡ a caminho da loja.`}
                          {linkedRide.status === 'EM_COLETA' && `Entregador ${linkedRide.driverName} estÃ¡ na loja conferindo os produtos do seu pedido.`}
                          {linkedRide.status === 'EM_TRANSITO' && `Entregador a caminho da sua entrega! PrevisÃ£o rÃ¡pida em Cachoeiras de Macacu.`}
                          {linkedRide.status === 'FINALIZADA' && 'Entrega concluÃ­da e finalizada com sucesso!'}
                        </p>

                        {linkedRide.driverName && (
                          <div className="pt-1.5 flex flex-wrap gap-2 text-[11px] text-emerald-300">
                            <span>ðŸ‘¤ <strong>Entregador:</strong> {linkedRide.driverName}</span>
                            {linkedRide.driverVehicleModel && (
                              <span>ðŸ›µ <strong>VeÃ­culo:</strong> {linkedRide.driverVehicleModel} ({linkedRide.driverVehiclePlate})</span>
                            )}
                          </div>
                        )}
                      </div>
                        ...
                        <div className="p-3 bg-white/10 rounded-xl border border-white/15 flex items-center justify-between">
                          <div>
                            <span className="text-[10px] text-amber-300 font-bold uppercase block">
                              CÃ³digo de ConfirmaÃ§Ã£o (Informe ao Entregador):
                            </span>
                          ...
                          <span className="text-[10px] text-slate-300 max-w-[180px] text-right">
                            Informe este cÃ³digo de 4 dÃ­gitos somente quando receber o pacote.
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    {order.status === 'ConcluÃ­do' && (
                      isOrderReviewedByCustomer(order.id) ? (
                        <span className="px-3 py-1.5 bg-emerald-50 text-emerald-700 font-bold rounded-xl border border-emerald-200 flex items-center space-x-1">
                          ...
                          <span>AvaliaÃ§Ã£o Realizada</span>
                        </span>
                      ) : (
                        <button
          ...
          {pickupOrders.map((ord) => (
            <div key={ord.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-3">
              ...
              <div className="p-4 bg-blue-50 border-2 border-dashed border-blue-300 rounded-xl flex items-center justify-between">
                <div>
                  <p className="text-xs text-blue-800 font-bold">Apresente este cÃ³digo no caixa:</p>
            ...
            <div key={ord.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800 text-xs font-bold">
                  ðŸ‘— Provador Agendado
                </span>
              </div>
              {ord.trialDetails && (
                <div className="p-3 bg-purple-50 rounded-xl text-xs space-y-1">
                  <p className="font-bold text-purple-950">Data: {ord.trialDetails.date} Ã s {ord.trialDetails.time}</p>
        ...
        <div className="space-y-4">
          {serviceBookings.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-2xl border border-slate-200">
              ...
              <p className="font-bold text-slate-700">Nenhum serviÃ§o ou consulta agendada no momento.</p>
              <p className="text-xs text-slate-400 mt-1">Explore os prestadores e consultÃ³rios no marketplace para agendar horÃ¡rios com total transparÃªncia.</p>
            </div>
          ) : (
            serviceBookings.map((ord) => {
              ...
              const isCompleted = ord.status === 'ConcluÃ­do' || resp?.status === 'CONCLUIDO';
                ...
                <div key={ord.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      >
                        â— {isConfirmed ? 'Confirmado pelo Prestador' : isRescheduled ? 'Novo HorÃ¡rio Proposto' : isCompleted ? 'Atendimento ConcluÃ­do' : isCancelled ? 'Cancelado / Recusado' : 'Aguardando Resposta'}
                      </span>
                    </div>
                  </div>
                  ...
                  {ord.serviceDetails && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                        <span className="text-[10px] font-bold text-slate-500 uppercase">ServiÃ§o Contratado</span>
                        ...
                        <div className="text-slate-600">
                          Modalidade: <strong>{ord.serviceDetails.serviceLocation === 'DOMICILIO' ? 'Em DomicÃ­lio' : ord.serviceDetails.serviceLocation === 'ONLINE' ? 'Online / Remoto' : 'No ConsultÃ³rio / Estabelecimento'}</strong>
                        </div>
                      </div>
                      ...
                      <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-1">
                        ...
                        <div className="font-bold text-blue-700 text-sm">
                          {resp?.confirmedDate || ord.serviceDetails.scheduledDate} Ã s {resp?.confirmedTime || ord.serviceDetails.scheduledTime}
                        </div>
                        <div className="text-slate-700 font-bold">
                          Valor: R$ {(resp?.confirmedPrice || ord.totalAmount || 0).toFixed(2).replace('.', ',')}
                          {ord.serviceDetails.pricingTypeSelected && (
                            <span className="text-[10px] text-slate-500 font-normal ml-1">
                              ({ord.serviceDetails.pricingTypeSelected === 'HORA' ? 'por hora' : ord.serviceDetails.pricingTypeSelected === 'DIARIA' ? 'diÃ¡ria' : ord.serviceDetails.pricingTypeSelected === 'MENSAL' ? 'mensalidade' : 'serviÃ§o'})
                            </span>
                          )}
                        </div>
                        {ord.customerAddress && (
                  ...
                  {/* Resposta Oficial do Prestador com OrientaÃ§Ãµes */}
                  {resp && (
                      ...
                      {resp.instructionsForCustomer && (
                        <div className="p-2.5 bg-white/70 rounded-lg border border-current/15 mt-2 space-y-1">
                          <strong className="block text-[11px] uppercase tracking-wide">
                            ðŸ“‹ OrientaÃ§Ãµes de Atendimento & Preparo:
                          </strong>
                  ...
                  {/* Actions (Mensagem Interna & AvaliaÃ§Ã£o) */}
        ...
        <div>
          {favoriteProducts.length === 0 ? (
            <div className="p-8 text-center bg-white rounded-2xl border border-slate-200">
              ...
              <p className="font-bold text-slate-700">VocÃª ainda nÃ£o favoritou nenhum produto.</p>
            </div>
          ) : (
      ...
      {/* TAB CONTENT: REPUTATION & AVALIAÃ‡Ã•ES */}
      {activeTab === 'reputation' && (
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h3 className="font-bold text-slate-900 text-base flex items-center space-x-2">
                  ...
                  <span>AvaliaÃ§Ãµes que vocÃª enviou para lojas e profissionais</span>
                </h3>
                <p className="text-xs text-slate-500">
                  OpiniÃµes pÃºblicas e notas concedidas por vocÃª para serviÃ§os e produtos do comÃ©rcio local.
                </p>
              </div>
              <button
            ...
            {reviews.filter((r) => r.userId === currentUser.id).length === 0 ? (
              <div className="p-8 text-center bg-slate-50 rounded-2xl border border-slate-200 text-slate-400">
                ...
                <p className="text-sm font-bold text-slate-600">
                  VocÃª ainda nÃ£o avaliou nenhum pedido concluÃ­do.
                </p>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <span className="text-xs text-slate-400 ml-2">
                            Pedido #{rev.orderCode} â€¢ {rev.createdAt}
                          </span>
                        </div>
