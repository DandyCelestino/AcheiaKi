...
export const ExclusiveAccessGate: React.FC<ExclusiveAccessGateProps> = ({
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Para o Portal do Vendedor, validaÃ§Ã£o comercial direta com as credenciais do vendedor
      ...
      if (res.requires2FA) {
        if (!isRoleAuthorized(res.user?.role, res.user?.merchantId)) {
          setErrorMessage('Esta conta nÃ£o possui privilÃ©gios para acessar este portal.');
          return;
        }
        setIs2FAStep(true);
        setSimulatedCode(res.simulated2FACode || '749210');
        setSuccessMessage(res.message || 'CÃ³digo de 2Âª etapa enviado com sucesso.');
        return;
      }
      ...
      if (res.success) {
        if (!isRoleAuthorized(res.user?.role, res.user?.merchantId)) {
          setErrorMessage(
            isSellerPortal
              ? 'Esta conta nÃ£o possui privilÃ©gios de consultor comercial. O acesso a este portal Ã© exclusivo para vendedores credenciados.'
              : 'Esta conta nÃ£o possui permissÃ£o para acessar este portal.'
          );
          return;
        }
        ...
        setErrorMessage(
          res.message ||
            (isSellerPortal
              ? 'Credenciais incorretas. Caso seja seu primeiro acesso, informe a senha padrÃ£o 12345678.'
              : 'Falha na autenticaÃ§Ã£o. Verifique suas credenciais.')
        );
      }
    } catch (err: any) {
    ...
    try {
      ...
      if (res.success) {
        if (res.user?.role !== requiredRole && requiredRole === 'MASTER') {
          setErrorMessage('A conta Google conectada nÃ£o possui nÃ­vel Master.');
          return;
        }
        setCurrentEnvironment(targetEnv);
      } else {
  ...
  const handleVerify2FA = (e: React.FormEvent) => {
    ...
    if (!twoFactorCode.trim() || twoFactorCode.trim().length < 6) {
      setErrorMessage('Digite o cÃ³digo de 6 dÃ­gitos recebido.');
      return;
    }

    const res = verifyTwoFactorCode(email.trim(), twoFactorCode.trim(), true);
    if (!res.success) {
      setErrorMessage(res.message || 'CÃ³digo de autenticaÃ§Ã£o em 2 etapas invÃ¡lido.');
    } else {
      setCurrentEnvironment(targetEnv);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-between text-slate-100 font-sans selection:bg-blue-600 selection:text-white">
      ...
      <header className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-extrabold text-sm tracking-tight text-white">
            Achei Aqui <span className="text-blue-400 font-normal">| SeguranÃ§a de Acessos</span>
          </span>
        </div>
        ...
        <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-black/60 relative overflow-hidden">
          ...
          {currentUser && !isRoleAuthorized(currentUser.role, currentUser.merchantId) && !is2FAStep && (
            <div className="mb-4 p-3.5 bg-slate-800/90 border border-slate-700 rounded-2xl text-xs space-y-2">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] text-slate-400">VocÃª estÃ¡ conectado como:</p>
                  <p className="font-bold text-white">
                    {currentUser.name} ({currentUser.role === 'CUSTOMER' ? 'UsuÃ¡rio / Cliente GrÃ¡tis R$ 0,00' : currentUser.role})
                  </p>
                </div>
                <div className="flex items-center space-x-2">
                  <button
              ...
              {currentUser.role === 'CUSTOMER' && (
                <div className="p-2.5 bg-blue-950/70 border border-blue-800/80 rounded-xl text-[11px] text-blue-200 space-y-1">
                  <p className="font-bold text-blue-100 flex items-center gap-1.5">
                    <span>â„¹ï¸</span> Perfil de UsuÃ¡rio Gratuito (R$ 0,00)
                  </p>
                  <p className="leading-relaxed">
                    O cadastro de usuÃ¡rio Ã© 100% gratuito e exclusivo para realizar compras e agendamentos no marketplace. UsuÃ¡rios nÃ£o possuem permissÃ£o para postar produtos, inserir banners ou interagir comercialmente. Para anunciar, solicite um credenciamento como <strong>Prestador de ServiÃ§os (R$ 29,90)</strong> ou <strong>Lojista</strong> com nossa equipe comercial.
                  </p>
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleVerify2FA} className="space-y-4">
              <div className="p-3 bg-slate-800 border border-slate-700 rounded-xl text-xs space-y-1.5">
                <div className="flex items-center space-x-2 text-amber-400 font-bold">
                  ...
                  <span>CÃ³digo de VerificaÃ§Ã£o 2FA:</span>
                </div>
                {simulatedCode && (
              ...
              >
                ...
                <span>Confirmar CÃ³digo 2FA</span>
              </button>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                ...
                {isSellerPortal && (
                  <p className="text-[11px] text-indigo-300/90 mt-1.5 flex items-center gap-1 bg-indigo-950/40 p-2 rounded-lg border border-indigo-900/50">
                    <span>ðŸ”‘</span>
                    <span>
                      Primeiro acesso? Digite a senha padrÃ£o <strong>12345678</strong>.
                    </span>
                  </p>
                )}
              </div>
              ...
              >
                {isSellerPortal ? (
                  ...
                  <>
                    <ShieldCheck className="w-4 h-4" />
                    <span>{isSubmitting ? 'Autenticando no Firebase...' : 'Autenticar com ConfirmaÃ§Ã£o em 2 Etapas'}</span>
                  </>
                )}
              </button>
                    ...
                    {salesAgents.map((sa) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setErrorMessage(null);
                          setSuccessMessage(`Credenciais de ${sa.name} preenchidas. Clique no botÃ£o de entrar.`);
                        }}
                      ...
                      >
                        <div className="truncate pr-2">
                          ...
                          <p className="text-[10px] text-slate-400 truncate">
                            {sa.email} â€¢ {sa.phone || 'Cachoeiras de Macacu'}
                          </p>
                        </div>
          ...
          <div className="mt-6 pt-4 border-t border-slate-800/80 text-center">
            <p className="text-[11px] text-slate-500 flex items-center justify-center gap-1.5">
              ...
              <span>Isolamento total de painÃ©is e criptografia de ponta a ponta</span>
            </p>
          </div>
        </div>
      </main>
      ...
      <footer className="px-6 py-4 text-center text-xs text-slate-600">
        Plataforma Achei Aqui â€¢ Cachoeiras de Macacu / RJ â€¢ Protocolo de SeguranÃ§a Rigoroso
      </footer>
    </div>
  );
};
