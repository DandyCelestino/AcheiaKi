...
export const MasterDeliveryView: React.FC = () => {
  ...
  const handleSaveTariffs = async (e: React.FormEvent) => {
    ...
    if (isNaN(rate) || rate <= 0 || isNaN(fee) || fee <= 0) {
      triggerToast('Valores de tarifas invÃ¡lidos.');
      return;
    }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 block">
                  RemuneraÃ§Ã£o do Entregador (R$ por KM)
                </label>
                ...
                <p className="text-[10px] text-slate-400">PadrÃ£o V1: R$ 1,00 / km</p>
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 block">
                  Taxa da Plataforma por SolicitaÃ§Ã£o (R$)
                </label>
                <div className="relative">
                  <span className="absolute left-3 top-2.5 text-xs text-slate-400 font-bold">R$</span>
                  <input
                ...
                <p className="text-[10px] text-slate-400">PadrÃ£o V1: R$ 2,00 por corrida</p>
              </div>
            </div>
